...
public class BitStream : ByteBuffer
{
    ...
    protected Vector3 CompressPoint;
    ...
    public void WriteNormalVector(Vector3 vec, byte bitCount)
    {
    ...
    public void ReadNormalVector(ref Vector3 vec, byte bitCount)
    {
    ...
    public static Vector3 DumbDownNormal(Vector3 vec, byte bitCount)
    {
    ...
    public void WriteNormalVector(Vector3 vec, byte angleBitCount, byte zBitCount)
    {
    ...
    public void ReadNormalVector(ref Vector3 vec, byte angleBitCount, byte zBitCount)
    {
        if (ReadFlag())
        {
    ...
    public void SetPointCompression(Vector3 p)
    {
    ...
    public void WritePointCompressed(Vector3 p, float scale)
    {
        var vec = new Vector3();
    ...
    public void ReadPointCompressed(ref Vector3 p, float scale)
    {
        var type = ReadInt(2);
        if (type == 3)
        {
    ...
    public void ReadStringTableEntry(out StringTableEntry ste)
    {
        ...
        else
        {
            ...
            ste.Set(buf.Contains('\0') ? buf[..buf.IndexOf('\0')] : buf);
        }
    }
    ...
    public void HashAndEncrypt(uint hashDigestSize, uint encryptStartOffset, SymmetricCipher theCipher)
    {
        ...
        var hash = SHA256.Create().ComputeHash(Data, 0, (int) digestStart);
    ...
    public bool DecryptAndCheckHash(uint hashDigestSize, uint decryptStartOffset, SymmetricCipher theCipher)
    {
        ...
        var hash = SHA256.Create().ComputeHash(buffer, 0, (int) (bufferSize - hashDigestSize));
