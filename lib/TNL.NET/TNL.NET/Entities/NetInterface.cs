...
public class NetInterface
{
    ...
    /// <summary>
    /// Constructs the interface without binding a UDP listen port. <see cref="Socket"/> is a
    /// live but unbound <see cref="TNLSocket"/>: sends work, no receive loop runs.
    /// </summary>
    public NetInterface(bool unbound)
    {
        if (!unbound)
            throw new ArgumentException("Use NetInterface(int port) to create a bound interface.", nameof(unbound));
    ...
    protected uint ComputeClientIdentityToken(IPEndPoint address, Nonce theNonce)
    {
        ...
        return BitConverter.ToUInt32(SHA256.Create().ComputeHash(buff), 0);
    }
