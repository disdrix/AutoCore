...
#pragma warning restore CA2211 // Non-constant fields should not be visible
    ...
    public void ClearMaskBits(ulong orMask)
    {
        if (_dirtyMaskBits > 0)
        {
            ...
            if (_dirtyMaskBits == 0)
            {
                // Head unlink must promote the static list head, mirroring the finalizer's
                // unlink above; writing _prevDirtyList here corrupts the static list (former
                // head keeps dangling links and later SetMaskBits orphans live entries).
                if (_prevDirtyList != null)
                    _prevDirtyList._nextDirtyList = _nextDirtyList;
                else
                    _dirtyList = _nextDirtyList;
