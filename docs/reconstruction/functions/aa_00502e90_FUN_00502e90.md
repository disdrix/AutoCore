# Function record: FUN_00502e90

| Field | Value |
|---|---|
| **Stable ID** | `aa_00502e90` |
| **Canonical name** | `FUN_00502e90` |
| **Address** | `0x00502e90` |
| **Module** | `autoassault.exe` (image base `0x400000`) |
| **System** | unknown |
| **Completion status** | **Partial scaffold** — raw/annotated/clean present; refine + dual review for important units |
| **Bit-for-bit / runtime / diff** | Open (deferred / not run) |

## Alias

Human name: `Inv_menu_equip_powerplant` (from string `menu_equip_powerplant`)

## Alias

Human name: `Inv_menu_equip_powerplant_00502e90` (from string `menu_equip_powerplant`)

## Purpose

Reconstructed from Ghidra decompile. See clean source and annotated notes for body.

## Signature (decompiler-derived)

```c
int * __thiscall FUN_00502e90(void *param_1,int *param_2)
```

## Artifacts

- Raw: `docs/reconstruction/raw/aa_00502e90_FUN_00502e90.md`
- Annotated: `docs/reconstruction/raw/aa_00502e90_FUN_00502e90.annotated.md`
- Clean: `docs/reconstruction/reconstructed-exact/FUN_00502e90.cpp`

## Callers / callees

(Populate from Ghidra xrefs in follow-up.)

## Confidence

| Claim | Level |
|---|---|
| Control flow from decompiler | High |
| Parameter semantic names | Probable / Tentative until caller pass |
| Types | Tentative |
