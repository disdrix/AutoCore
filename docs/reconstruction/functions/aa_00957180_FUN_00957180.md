# Function record: FUN_00957180

| Field | Value |
|---|---|
| **Stable ID** | `aa_00957180` |
| **Canonical name** | `FUN_00957180` |
| **Address** | `0x00957180` |
| **Module** | `autoassault.exe` (image base `0x400000`) |
| **System** | unknown |
| **Completion status** | **Partial scaffold** — raw/annotated/clean present; refine + dual review for important units |
| **Bit-for-bit / runtime / diff** | Open (deferred / not run) |

## Alias

Human name: `Skill_Skill_d_s` (from string `Skill %d: (%s)

`)

## Alias

Human name: `Skill_Skill_d_s_00957180` (from string `Skill %d: (%s)

`)

## Purpose

Reconstructed from Ghidra decompile. See clean source and annotated notes for body.

## Signature (decompiler-derived)

```c
uint32_t /* width from decompiler */ __thiscall FUN_00957180(uint32_t /* width from decompiler */ param_1,uint32_t /* width from decompiler */ param_2,char *param_3)
```

## Artifacts

- Raw: `docs/reconstruction/raw/aa_00957180_FUN_00957180.md`
- Annotated: `docs/reconstruction/raw/aa_00957180_FUN_00957180.annotated.md`
- Clean: `docs/reconstruction/reconstructed-exact/FUN_00957180.cpp`

## Callers / callees

(Populate from Ghidra xrefs in follow-up.)

## Confidence

| Claim | Level |
|---|---|
| Control flow from decompiler | High |
| Parameter semantic names | Probable / Tentative until caller pass |
| Types | Tentative |
