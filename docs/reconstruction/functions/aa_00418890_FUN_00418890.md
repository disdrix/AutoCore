# Function record: FUN_00418890

| Field | Value |
|---|---|
| **Stable ID** | `aa_00418890` |
| **Canonical name** | `FUN_00418890` |
| **Address** | `0x00418890` |
| **Module** | `autoassault.exe` (image base `0x400000`) |
| **System** | unknown |
| **Completion status** | **Partial scaffold** — raw/annotated/clean present; refine + dual review for important units |
| **Bit-for-bit / runtime / diff** | Open (deferred / not run) |

## Alias

Human name: `Named_CalleeOf_Client_RecvSkillStatusEffect_00418890` (callee of `Client_RecvSkillStatusEffect`)

## Purpose

Reconstructed from Ghidra decompile. See clean source and annotated notes for body.

## Signature (decompiler-derived)

```c
void __thiscall FUN_00418890(int param_1,int *param_2,int *param_3)
```

## Artifacts

- Raw: `docs/reconstruction/raw/aa_00418890_FUN_00418890.md`
- Annotated: `docs/reconstruction/raw/aa_00418890_FUN_00418890.annotated.md`
- Clean: `docs/reconstruction/reconstructed-exact/FUN_00418890.cpp`

## Callers / callees

(Populate from Ghidra xrefs in follow-up.)

## Confidence

| Claim | Level |
|---|---|
| Control flow from decompiler | High |
| Parameter semantic names | Probable / Tentative until caller pass |
| Types | Tentative |
