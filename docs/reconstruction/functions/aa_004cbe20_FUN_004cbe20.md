# Function record: FUN_004cbe20

| Field | Value |
|---|---|
| **Stable ID** | `aa_004cbe20` |
| **Canonical name** | `FUN_004cbe20` |
| **Address** | `0x004cbe20` |
| **Module** | `autoassault.exe` (image base `0x400000`) |
| **System** | unknown |
| **Completion status** | **Partial scaffold** — raw/annotated/clean present; refine + dual review for important units |
| **Bit-for-bit / runtime / diff** | Open (deferred / not run) |

## Alias

Human name: `Named_CalleeOf_Named_CalleeOf_CVOGHBAIBase_GetTargetFromAggro_004cbe20` (callee of `Named_CalleeOf_CVOGHBAIBase_GetTargetFromAggro`)

## Purpose

Reconstructed from Ghidra decompile. See clean source and annotated notes for body.

## Signature (decompiler-derived)

```c
void __thiscall FUN_004cbe20(int param_1,uint32_t /* width from decompiler */ *param_2,int *param_3)
```

## Artifacts

- Raw: `docs/reconstruction/raw/aa_004cbe20_FUN_004cbe20.md`
- Annotated: `docs/reconstruction/raw/aa_004cbe20_FUN_004cbe20.annotated.md`
- Clean: `docs/reconstruction/reconstructed-exact/FUN_004cbe20.cpp`

## Callers / callees

(Populate from Ghidra xrefs in follow-up.)

## Confidence

| Claim | Level |
|---|---|
| Control flow from decompiler | High |
| Parameter semantic names | Probable / Tentative until caller pass |
| Types | Tentative |
