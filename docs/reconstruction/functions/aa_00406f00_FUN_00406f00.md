# Function record: FUN_00406f00

| Field | Value |
|---|---|
| **Stable ID** | `aa_00406f00` |
| **Canonical name** | `FUN_00406f00` |
| **Address** | `0x00406f00` |
| **Module** | `autoassault.exe` (image base `0x400000`) |
| **System** | unknown |
| **Completion status** | **Partial scaffold** — raw/annotated/clean present; refine + dual review for important units |
| **Bit-for-bit / runtime / diff** | Open (deferred / not run) |

## Alias

Human name: `Named_CalleeOf_Skill_Uses_SkillSet_GetEntryCount_00406f00` (callee of `Skill_Uses_SkillSet_GetEntryCount`)

## Purpose

Reconstructed from Ghidra decompile. See clean source and annotated notes for body.

## Signature (decompiler-derived)

```c
int FUN_00406f00(void)
```

## Artifacts

- Raw: `docs/reconstruction/raw/aa_00406f00_FUN_00406f00.md`
- Annotated: `docs/reconstruction/raw/aa_00406f00_FUN_00406f00.annotated.md`
- Clean: `docs/reconstruction/reconstructed-exact/FUN_00406f00.cpp`

## Callers / callees

(Populate from Ghidra xrefs in follow-up.)

## Confidence

| Claim | Level |
|---|---|
| Control flow from decompiler | High |
| Parameter semantic names | Probable / Tentative until caller pass |
| Types | Tentative |
