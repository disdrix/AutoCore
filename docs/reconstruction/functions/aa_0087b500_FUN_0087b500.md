# Function record: FUN_0087b500

| Field | Value |
|---|---|
| **Stable ID** | `aa_0087b500` |
| **Canonical name** | `FUN_0087b500` |
| **Address** | `0x0087b500` |
| **Module** | `autoassault.exe` (image base `0x400000`) |
| **System** | unknown |
| **Completion status** | **Partial scaffold** — raw/annotated/clean present; refine + dual review for important units |
| **Bit-for-bit / runtime / diff** | Open (deferred / not run) |

## Alias

Human name: `Named_CalleeOf_Mission_Client_ClaimSelectedReward_0087b500` (callee of `Mission_Client_ClaimSelectedReward`)

## Purpose

Reconstructed from Ghidra decompile. See clean source and annotated notes for body.

## Signature (decompiler-derived)

```c
uint32_t /* width from decompiler */ __thiscall FUN_0087b500(int *param_1,int param_2,uint32_t /* width from decompiler */ param_3)
```

## Artifacts

- Raw: `docs/reconstruction/raw/aa_0087b500_FUN_0087b500.md`
- Annotated: `docs/reconstruction/raw/aa_0087b500_FUN_0087b500.annotated.md`
- Clean: `docs/reconstruction/reconstructed-exact/FUN_0087b500.cpp`

## Callers / callees

(Populate from Ghidra xrefs in follow-up.)

## Confidence

| Claim | Level |
|---|---|
| Control flow from decompiler | High |
| Parameter semantic names | Probable / Tentative until caller pass |
| Types | Tentative |
