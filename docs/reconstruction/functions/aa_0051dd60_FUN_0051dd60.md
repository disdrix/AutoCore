# Function record: FUN_0051dd60

| Field | Value |
|---|---|
| **Stable ID** | `aa_0051dd60` |
| **Canonical name** | `FUN_0051dd60` |
| **Address** | `0x0051dd60` |
| **Module** | `autoassault.exe` (image base `0x400000`) |
| **System** | unknown |
| **Completion status** | **Partial scaffold** — raw/annotated/clean present; refine + dual review for important units |
| **Bit-for-bit / runtime / diff** | Open (deferred / not run) |

## Alias

Human name: `Named_CalleeOf_Named_CalleeOf_Named_CalleeOf_Skill_Uses_Skill_E_0051dd60` (callee of `Named_CalleeOf_Named_CalleeOf_Skill_Uses_Skill_EnsureLoadedInTr`)

## Purpose

Reconstructed from Ghidra decompile. See clean source and annotated notes for body.

## Signature (decompiler-derived)

```c
uint32_t /* width from decompiler */ * __thiscall FUN_0051dd60(uint32_t /* width from decompiler */ *param_1,byte param_2)
```

## Artifacts

- Raw: `docs/reconstruction/raw/aa_0051dd60_FUN_0051dd60.md`
- Annotated: `docs/reconstruction/raw/aa_0051dd60_FUN_0051dd60.annotated.md`
- Clean: `docs/reconstruction/reconstructed-exact/FUN_0051dd60.cpp`

## Callers / callees

(Populate from Ghidra xrefs in follow-up.)

## Confidence

| Claim | Level |
|---|---|
| Control flow from decompiler | High |
| Parameter semantic names | Probable / Tentative until caller pass |
| Types | Tentative |
