# Function record: FUN_005d8e30

| Field | Value |
|---|---|
| **Stable ID** | `aa_005d8e30` |
| **Canonical name** | `FUN_005d8e30` |
| **Address** | `0x005d8e30` |
| **Module** | `autoassault.exe` (image base `0x400000`) |
| **System** | unknown |
| **Completion status** | **Partial scaffold** — raw/annotated/clean present; refine + dual review for important units |
| **Bit-for-bit / runtime / diff** | Open (deferred / not run) |

## Alias

Human name: `Named_Server_deleted_005d8e30` (from string `Server deleted`)

## Purpose

Reconstructed from Ghidra decompile. See clean source and annotated notes for body.

## Signature (decompiler-derived)

```c
void __fastcall FUN_005d8e30(uint32_t /* width from decompiler */ *param_1)
```

## Artifacts

- Raw: `docs/reconstruction/raw/aa_005d8e30_FUN_005d8e30.md`
- Annotated: `docs/reconstruction/raw/aa_005d8e30_FUN_005d8e30.annotated.md`
- Clean: `docs/reconstruction/reconstructed-exact/FUN_005d8e30.cpp`

## Callers / callees

(Populate from Ghidra xrefs in follow-up.)

## Confidence

| Claim | Level |
|---|---|
| Control flow from decompiler | High |
| Parameter semantic names | Probable / Tentative until caller pass |
| Types | Tentative |
