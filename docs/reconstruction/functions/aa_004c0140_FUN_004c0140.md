# Function record: FUN_004c0140

| Field | Value |
|---|---|
| **Stable ID** | `aa_004c0140` |
| **Canonical name** | `FUN_004c0140` |
| **Address** | `0x004c0140` |
| **Module** | `autoassault.exe` (image base `0x400000`) |
| **System** | unknown |
| **Completion status** | **Partial scaffold** — raw/annotated/clean present; refine + dual review for important units |
| **Bit-for-bit / runtime / diff** | Open (deferred / not run) |

## Alias

Human name: `Named_CalleeOf_Named_CalleeOf_CVOGCharacter_CreateFromPacket_004c0140` (callee of `Named_CalleeOf_CVOGCharacter_CreateFromPacket`)

## Purpose

Reconstructed from Ghidra decompile. See clean source and annotated notes for body.

## Signature (decompiler-derived)

```c
uint FUN_004c0140(int param_1,int param_2,uint32_t /* width from decompiler */ param_3,uint32_t /* width from decompiler */ param_4,uint32_t /* width from decompiler */ param_5,
```

## Artifacts

- Raw: `docs/reconstruction/raw/aa_004c0140_FUN_004c0140.md`
- Annotated: `docs/reconstruction/raw/aa_004c0140_FUN_004c0140.annotated.md`
- Clean: `docs/reconstruction/reconstructed-exact/FUN_004c0140.cpp`

## Callers / callees

(Populate from Ghidra xrefs in follow-up.)

## Confidence

| Claim | Level |
|---|---|
| Control flow from decompiler | High |
| Parameter semantic names | Probable / Tentative until caller pass |
| Types | Tentative |
