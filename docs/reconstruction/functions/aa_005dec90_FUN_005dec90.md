# Function record: FUN_005dec90

| Field | Value |
|---|---|
| **Stable ID** | `aa_005dec90` |
| **Canonical name** | `FUN_005dec90` |
| **Address** | `0x005dec90` |
| **Module** | `autoassault.exe` (image base `0x400000`) |
| **System** | unknown |
| **Completion status** | **Partial scaffold** — raw/annotated/clean present; refine + dual review for important units |
| **Bit-for-bit / runtime / diff** | Open (deferred / not run) |

## Alias

Human name: `Named_Base_system_initialized` (from plate string ` Base system initialized.   
`)

## Purpose

Reconstructed from Ghidra decompile. See clean source and annotated notes for body.

## Signature (decompiler-derived)

```c
void FUN_005dec90(void)
```

## Artifacts

- Raw: `docs/reconstruction/raw/aa_005dec90_FUN_005dec90.md`
- Annotated: `docs/reconstruction/raw/aa_005dec90_FUN_005dec90.annotated.md`
- Clean: `docs/reconstruction/reconstructed-exact/FUN_005dec90.cpp`

## Callers / callees

(Populate from Ghidra xrefs in follow-up.)

## Confidence

| Claim | Level |
|---|---|
| Control flow from decompiler | High |
| Parameter semantic names | Probable / Tentative until caller pass |
| Types | Tentative |
