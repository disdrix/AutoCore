# Function record: FUN_00787f90

| Field | Value |
|---|---|
| **Stable ID** | `aa_00787f90` |
| **Canonical name** | `FUN_00787f90` |
| **Address** | `0x00787f90` |
| **Module** | `autoassault.exe` (image base `0x400000`) |
| **System** | unknown |
| **Completion status** | **Partial scaffold** — raw/annotated/clean present; refine + dual review for important units |
| **Bit-for-bit / runtime / diff** | Open (deferred / not run) |

## Alias

Human name: `Named_CalleeOf_Mission_Uses_CVOGReaction_FailMissionNotify_00787f90` (callee of `Mission_Uses_CVOGReaction_FailMissionNotify`)

## Purpose

Reconstructed from Ghidra decompile. See clean source and annotated notes for body.

## Signature (decompiler-derived)

```c
uint FUN_00787f90(float *param_1,uint32_t /* width from decompiler */ *param_2,uint32_t /* width from decompiler */ *param_3,uint32_t /* width from decompiler */ *param_4)
```

## Artifacts

- Raw: `docs/reconstruction/raw/aa_00787f90_FUN_00787f90.md`
- Annotated: `docs/reconstruction/raw/aa_00787f90_FUN_00787f90.annotated.md`
- Clean: `docs/reconstruction/reconstructed-exact/FUN_00787f90.cpp`

## Callers / callees

(Populate from Ghidra xrefs in follow-up.)

## Confidence

| Claim | Level |
|---|---|
| Control flow from decompiler | High |
| Parameter semantic names | Probable / Tentative until caller pass |
| Types | Tentative |
