# Function record: FUN_0051b550

| Field | Value |
|---|---|
| **Stable ID** | `aa_0051b550` |
| **Canonical name** | `FUN_0051b550` |
| **Address** | `0x0051b550` |
| **Module** | `autoassault.exe` (image base `0x400000`) |
| **System** | unknown |
| **Completion status** | **Partial scaffold** — raw/annotated/clean present; refine + dual review for important units |
| **Bit-for-bit / runtime / diff** | Open (deferred / not run) |

## Alias

Human name: `Named_CalleeOf_Skill_Uses_Skill_EnsureLoadedInTree_0051b550` (callee of `Skill_Uses_Skill_EnsureLoadedInTree`)

## Purpose

Reconstructed from Ghidra decompile. See clean source and annotated notes for body.

## Signature (decompiler-derived)

```c
void __fastcall FUN_0051b550(int *param_1)
```

## Artifacts

- Raw: `docs/reconstruction/raw/aa_0051b550_FUN_0051b550.md`
- Annotated: `docs/reconstruction/raw/aa_0051b550_FUN_0051b550.annotated.md`
- Clean: `docs/reconstruction/reconstructed-exact/FUN_0051b550.cpp`

## Callers / callees

(Populate from Ghidra xrefs in follow-up.)

## Confidence

| Claim | Level |
|---|---|
| Control flow from decompiler | High |
| Parameter semantic names | Probable / Tentative until caller pass |
| Types | Tentative |
