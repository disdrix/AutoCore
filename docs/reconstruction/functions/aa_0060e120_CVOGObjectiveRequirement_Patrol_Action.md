# Function record: CVOGObjectiveRequirement_Patrol_Action

| Field | Value |
|---|---|
| **Stable ID** | `aa_0060e120` |
| **Canonical name** | `CVOGObjectiveRequirement_Patrol_Action` |
| **Address** | `0x0060e120` |
| **Module** | `autoassault.exe` (image base `0x400000`) |
| **System** | missions-progression |
| **Completion status** | **Dual A/B present** — clean + raw; runtime/diff/bit-exact open |
| **Bit-for-bit / runtime / diff** | Open |

## Purpose

Patrol action: progress fraction slot/(required*weight) clamped to 1.0 (0 denom -> 1.0).

## Related artifacts

| Kind | Path |
|---|---|
| Raw | `raw/aa_0060e120_CVOGObjectiveRequirement_Patrol_Action.md` |
| Annotated | `raw/aa_0060e120_CVOGObjectiveRequirement_Patrol_Action.annotated.md` |
| Clean | `reconstructed-exact/CVOGObjectiveRequirement_Patrol_Action.cpp` |
| Dual A | `reviews/A_aa_0060e120_CVOGObjectiveRequirement_Patrol_Action.md` |
| Dual B | `reviews/B_aa_0060e120_CVOGObjectiveRequirement_Patrol_Action.md` |

## Confidence

| Claim | Level |
|---|---|
| Control flow from decompiler | High (see dual A) |
| Parameter semantic names | Probable / Tentative (see dual B) |
| Runtime / bit-exact | Open |

## Open questions

- See dual B open questions
- Runtime observation / differential equivalence
