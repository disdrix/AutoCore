# Function record: FUN_00546060

| Field | Value |
|---|---|
| **Stable ID** | `aa_00546060` |
| **Canonical name** | `FUN_00546060` |
| **Address** | `0x00546060` |
| **Module** | `autoassault.exe` (image base `0x400000`) |
| **System** | unknown |
| **Completion status** | **Partial scaffold** — raw/annotated/clean present; refine + dual review for important units |
| **Bit-for-bit / runtime / diff** | Open (deferred / not run) |

## Alias

Human name: `Named_CalleeOf_Mission_Mission_lacking_child_nodes_s_00546060` (callee of `Mission_Mission_lacking_child_nodes_s`)

## Purpose

Reconstructed from Ghidra decompile. See clean source and annotated notes for body.

## Signature (decompiler-derived)

```c
void __thiscall FUN_00546060(int param_1,char *param_2)
```

## Artifacts

- Raw: `docs/reconstruction/raw/aa_00546060_FUN_00546060.md`
- Annotated: `docs/reconstruction/raw/aa_00546060_FUN_00546060.annotated.md`
- Clean: `docs/reconstruction/reconstructed-exact/FUN_00546060.cpp`

## Callers / callees

(Populate from Ghidra xrefs in follow-up.)

## Confidence

| Claim | Level |
|---|---|
| Control flow from decompiler | High |
| Parameter semantic names | Probable / Tentative until caller pass |
| Types | Tentative |
