# Function record: FUN_00547050

| Field | Value |
|---|---|
| **Stable ID** | `aa_00547050` |
| **Canonical name** | `FUN_00547050` |
| **Address** | `0x00547050` |
| **Module** | `autoassault.exe` (image base `0x400000`) |
| **System** | unknown |
| **Completion status** | **Partial scaffold** — raw/annotated/clean present; refine + dual review for important units |
| **Bit-for-bit / runtime / diff** | Open (deferred / not run) |

## Alias

Human name: `Mission_Mission_Sequence_too_big_d_d_s` (from string `Mission - Sequence too big.. (%d >= %d) %s`)

## Alias

Human name: `Mission_Mission_Sequence_too_big_d_d_s_00547050` (from string `Mission - Sequence too big.. (%d >= %d) %s`)

## Purpose

Reconstructed from Ghidra decompile. See clean source and annotated notes for body.

## Signature (decompiler-derived)

```c
/* WARNING: Removing unreachable block (ram,0x00547104) */
```

## Artifacts

- Raw: `docs/reconstruction/raw/aa_00547050_FUN_00547050.md`
- Annotated: `docs/reconstruction/raw/aa_00547050_FUN_00547050.annotated.md`
- Clean: `docs/reconstruction/reconstructed-exact/FUN_00547050.cpp`

## Callers / callees

(Populate from Ghidra xrefs in follow-up.)

## Confidence

| Claim | Level |
|---|---|
| Control flow from decompiler | High |
| Parameter semantic names | Probable / Tentative until caller pass |
| Types | Tentative |
