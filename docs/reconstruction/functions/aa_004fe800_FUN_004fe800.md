# Function record: FUN_004fe800

| Field | Value |
|---|---|
| **Stable ID** | `aa_004fe800` |
| **Canonical name** | `FUN_004fe800` |
| **Address** | `0x004fe800` |
| **Module** | `autoassault.exe` (image base `0x400000`) |
| **System** | unknown |
| **Completion status** | **Partial scaffold** — raw/annotated/clean present; refine + dual review for important units |
| **Bit-for-bit / runtime / diff** | Open (deferred / not run) |

## Alias

Human name: `Combat_Set_weapon_melee_unhappy_type_CBID` (from plate string `Set weapon melee of unhappy type: CBID:%d COID:%I64d`)

## Alias_prev

Human name: `Named_CalleeOf_Client_RecvInventoryEquip_004fe800` (callee of `Client_RecvInventoryEquip`)

## Purpose

Reconstructed from Ghidra decompile. See clean source and annotated notes for body.

## Signature (decompiler-derived)

```c
void __thiscall FUN_004fe800(int param_1,int param_2,uint32_t /* width from decompiler */ *param_3,char param_4)
```

## Artifacts

- Raw: `docs/reconstruction/raw/aa_004fe800_FUN_004fe800.md`
- Annotated: `docs/reconstruction/raw/aa_004fe800_FUN_004fe800.annotated.md`
- Clean: `docs/reconstruction/reconstructed-exact/FUN_004fe800.cpp`

## Callers / callees

(Populate from Ghidra xrefs in follow-up.)

## Confidence

| Claim | Level |
|---|---|
| Control flow from decompiler | High |
| Parameter semantic names | Probable / Tentative until caller pass |
| Types | Tentative |
