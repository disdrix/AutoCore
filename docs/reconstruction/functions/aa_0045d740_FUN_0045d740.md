# Function record: FUN_0045d740

| Field | Value |
|---|---|
| **Stable ID** | `aa_0045d740` |
| **Canonical name** | `FUN_0045d740` |
| **Address** | `0x0045d740` |
| **Module** | `autoassault.exe` (image base `0x400000`) |
| **System** | unknown |
| **Completion status** | **Partial scaffold** — raw/annotated/clean present; refine + dual review for important units |
| **Bit-for-bit / runtime / diff** | Open (deferred / not run) |

## Alias

Human name: `Named_CalleeOf_Named_CalleeOf_Named_CalleeOf_Named_CalleeOf_Nam_0045d740` (callee of `Named_CalleeOf_Named_CalleeOf_Named_CalleeOf_Named_anmTrackMast`)

## Purpose

Reconstructed from Ghidra decompile. See clean source and annotated notes for body.

## Signature (decompiler-derived)

```c
void FUN_0045d740(uint32_t /* width from decompiler */ param_1,uint32_t /* width from decompiler */ param_2,uint32_t /* width from decompiler */ param_3,uint8_t param_4)
```

## Artifacts

- Raw: `docs/reconstruction/raw/aa_0045d740_FUN_0045d740.md`
- Annotated: `docs/reconstruction/raw/aa_0045d740_FUN_0045d740.annotated.md`
- Clean: `docs/reconstruction/reconstructed-exact/FUN_0045d740.cpp`

## Callers / callees

(Populate from Ghidra xrefs in follow-up.)

## Confidence

| Claim | Level |
|---|---|
| Control flow from decompiler | High |
| Parameter semantic names | Probable / Tentative until caller pass |
| Types | Tentative |
