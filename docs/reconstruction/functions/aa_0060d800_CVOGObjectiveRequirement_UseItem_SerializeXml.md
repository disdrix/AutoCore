# Function record: CVOGObjectiveRequirement_UseItem_SerializeXml

| Field | Value |
|---|---|
| **Stable ID** | `aa_0060d800` |
| **Canonical name** | `CVOGObjectiveRequirement_UseItem_SerializeXml` |
| **Address** | `0x0060d800` |
| **Module** | `autoassault.exe` (image base `0x400000`) |
| **System** | missions-progression |
| **Completion status** | **Dual A/B present** — clean + raw; runtime/diff/bit-exact open |
| **Bit-for-bit / runtime / diff** | Open |

## Purpose

COM/XML property load for UseItem fields (tag->offset map).

## Related artifacts

| Kind | Path |
|---|---|
| Raw | `raw/aa_0060d800_CVOGObjectiveRequirement_UseItem_SerializeXml.md` |
| Annotated | `raw/aa_0060d800_CVOGObjectiveRequirement_UseItem_SerializeXml.annotated.md` |
| Clean | `reconstructed-exact/CVOGObjectiveRequirement_UseItem_SerializeXml.cpp` |
| Dual A | `reviews/A_aa_0060d800_CVOGObjectiveRequirement_UseItem_SerializeXml.md` |
| Dual B | `reviews/B_aa_0060d800_CVOGObjectiveRequirement_UseItem_SerializeXml.md` |

## Confidence

| Claim | Level |
|---|---|
| Control flow from decompiler | High (see dual A) |
| Parameter semantic names | Probable / Tentative (see dual B) |
| Runtime / bit-exact | Open |

## Open questions

- See dual B open questions
- Runtime observation / differential equivalence
