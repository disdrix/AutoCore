// =============================================================================
// FUN_0053a260  (twin of Map_EraseRange_Val12)
// -----------------------------------------------------------------------------
// Stable ID: aa_0053a260
// Address:   0x0053a260
// Prefer named clean: Map_EraseRange_Val12.cpp
// W28-S dual seal 2026-07-29
// =============================================================================

#include <cstdint>

void __thiscall FUN_0053a260(int param_1, uint32_t *param_2, int *param_3, int *param_4)
{
  char cVar1;
  int *piVar2;
  int *piVar3;
  int *piVar4;
  int *piVar5;
  int *piVar6;

  piVar4 = param_4;
  piVar6 = *(int **)(param_1 + 4);
  piVar3 = param_3;
  if ((param_3 == (int *)*piVar6) && (param_4 == piVar6)) {
    FUN_00538690((void *)piVar6[1]);
    *(int *)(*(int *)(param_1 + 4) + 4) = *(int *)(param_1 + 4);
    *(uint32_t *)(param_1 + 8) = 0;
    *(uint32_t *)*(uint32_t *)(param_1 + 4) = *(uint32_t *)(param_1 + 4);
    *(int *)(*(int *)(param_1 + 4) + 8) = *(int *)(param_1 + 4);
    *param_2 = **(uint32_t **)(param_1 + 4);
    return;
  }
  while (piVar3 != piVar4) {
    piVar6 = piVar3;
    if (*(char *)((int)piVar3 + 0x19) == '\0') {
      piVar6 = (int *)piVar3[2];
      if (*(char *)((int)piVar6 + 0x19) == '\0') {
        cVar1 = *(char *)(*piVar6 + 0x19);
        piVar2 = (int *)*piVar6;
        while (cVar1 == '\0') {
          cVar1 = *(char *)(*piVar2 + 0x19);
          piVar6 = piVar2;
          piVar2 = (int *)*piVar2;
        }
      } else {
        cVar1 = *(char *)(piVar3[1] + 0x19);
        piVar5 = (int *)piVar3[1];
        piVar2 = piVar3;
        while ((piVar6 = piVar5, cVar1 == '\0' && (piVar2 == (int *)piVar6[2]))) {
          cVar1 = *(char *)(piVar6[1] + 0x19);
          piVar5 = (int *)piVar6[1];
          piVar2 = piVar6;
        }
      }
    }
    FUN_005399f0(&param_3, piVar3);
    piVar3 = piVar6;
  }
  *param_2 = (uint32_t)piVar3;
  return;
}
