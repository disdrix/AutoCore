// =============================================================================
// gfxGeometryPiece_Serialize
// -----------------------------------------------------------------------------
// Purpose:  Serialize one mesh section as a GPCE ("ECPG") chunk, version 10.
//           Child of GBOD ("DOBG"). Writes effect, INDX, VERT, BBOX, names, USDA.
//
// Address:  0x0074ac40  (autoassault.exe, image base 0x400000)
// Stable:   aa_0074ac40
// System:   graphics / gfx (asset I/O)
// Source:   palantir/graphics/gfxGeometryPiece.cpp
//
// Control flow summary:
//   1) stoChunkWriter_BeginChunk(0x47504345 /*GPCE*/, 10)
//   2) write flags / effect path (FUN_004399a0, FUN_0096f890)
//   3) index: this[3] -> gfxIndexBufferImpl_Serialize; else success-1
//   4) vertex: morph path (this[7]) vs plain this[5] -> gfxVertexBufferImpl_Serialize
//   5) optional vtbl+0x5c when flag bit0 @ this+0x2f
//   6) BBOX FUN_0074bda0; name strings; USDA tail (LODLevel)
//   7) EndChunk; OR all status bits
//
// Chunk order: has-effect, EFCT, INDX, VERT, BBOX (XOBB), names, USDA (ADSU)
// VERT/INDX paired per piece (exactly one of each).
//
// Exactness: CF mirrors raw (unreachable-block warnings only). Runtime open.
// =============================================================================
...
uint __thiscall gfxGeometryPiece_Serialize(int *param_1,uint32_t /* width from decompiler */ *param_2)
...

{
  ...
  if (param_1[7] == 0) {
    ...
    for (; local_20 != (uint16_t *)(local_1c + -8); local_20 = local_20 + 4) {
    ...
    if (param_1[5] == 0) {
      ...
      uVar4 = *(uint32_t /* width from decompiler */ *)(param_1[5] + 0x18);
