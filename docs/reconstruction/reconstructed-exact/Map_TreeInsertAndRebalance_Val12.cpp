// =============================================================================
// Map_TreeInsertAndRebalance_Val12  (FUN_0051c1c0)
// -----------------------------------------------------------------------------
// Stable ID: aa_0051c1c0
// Address:   0x0051c1c0  (autoassault.exe, image base 0x400000)
...
//   EAX left as outIt* after *outIt = new node (caller treats return as holder)
//
// THROW
//   if size > 0x15555553 → length_error "map/set<T> too long" (0x00a1530c)
//   via _CxxThrowException(..., &DAT_00acc388)
//
// SOLE LIVE CALLER (this image)
//   Skill_CategoryCooldownMap_Insert @ 0x0051d2f0 (xrefs 0051d349, 0051d380)
// =============================================================================
...
struct MapNode_Val12 {
  ...
  int32_t v0;             // +0x0C  (key for category-cooldown maps)
  int32_t v1;             // +0x10
  int32_t v2;             // +0x14
...
// External callees (roles sealed elsewhere; product names open for rotates)
...
extern "C" void FUN_00573170(MapNode_Val12 *node); // rotate helper
extern "C" void FUN_00418c10(MapNode_Val12 *node); // rotate helper
...
{
  // SEH frame omitted (LAB_009a3502); present in retail for throw path.
  ...
  // Node: new(0x1c), color red (0), isnil 0; value copy 3 dwords.
  MapNode_Val12 *n =
  ...
  for (;;) {
    ...
    if (parent == uncle) {
      ...
      if (uncle->color == 0) {
        ...
        if (x == parent->right) {
          FUN_00573170(parent); // rotate at parent
          x = parent;
          // parent re-read from x after rotate in retail path
        }
        x->parent->color = 1;
        x->parent->parent->color = 0;
        FUN_00418c10(x->parent->parent); // rotate at grand
      }
    } else {
      ...
      if (uncle->color == 0) {
        ...
        x->parent->color = 1;
        x->parent->parent->color = 0;
        // Inlined rotate of grandparent (isnil guard @ +0x19)
        MapNode_Val12 *g = x->parent->parent;
...
void __thiscall FUN_0051c1c0(MapShell_Val12 *map, MapNode_Val12 **outIt,
                             char addLeft, MapNode_Val12 *where,
                             const Val12 *value) {
  Map_TreeInsertAndRebalance_Val12(map, outIt, addLeft, where, value);
}
