// =============================================================================
// PalantirEnv_InitPhases_Inferred
// -----------------------------------------------------------------------------
// Stable ID: aa_0048fc90
// Address:   0x0048fc90-0x0048fed0 exclusive (576 B / 0x240)
// System:    client / VOGEnvironmentSector phases on Palantir env host
...
//
// PURPOSE:
// Phase bootstrap method on the 0x198 env host (ECX = parent+0xE894).
// Sole caller Client_InitPalantirViewBundle_Inferred after view↔env cross-wire:
//   FUN_0048fc90(env, 1).
// Lazy-inits Water + Distort process singletons (view@+0xC0); always builds
// three owned phases from parent@+0xB8; loads NDRiver.fx; returns 0 (RET 4).
// Reject scaffold name Drive_NDRiver_fx (tail string only).
...
uint32_t __thiscall PalantirEnv_InitPhases_Inferred(void *env, void *flag)
{
  ...
  if (DAT_00b04808 == nullptr) {
    ...
    DAT_00b0480c = env;
    {
      ...
      reinterpret_cast<Vcall>(vt[8])(DAT_00b04808); // vtbl+0x20
    }
    hr = FUN_00576860();
    if (hr < 0) {
      vog_LogMessage(
