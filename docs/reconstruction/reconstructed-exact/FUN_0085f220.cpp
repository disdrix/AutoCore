// =============================================================================
// Client_UI_InventoryDropHitTest  (Ghidra: FUN_0085f220)
// -----------------------------------------------------------------------------
...
// Dual: docs/reconstruction/reviews/A_aa_0085f220_Client_UI_InventoryDropHitTest.md
//       docs/reconstruction/reviews/B_aa_0085f220_Client_UI_InventoryDropHitTest.md
// =============================================================================

// PURPOSE
//   Drop hit-test for inventory UI grids (primary caller: Client_UI_InventoryDropToGrid).
//   Maps pointer -> page-local cell (X,Y), clamps for held-item footprint, gates with
//   InventoryGrid_CanPlace using absolute Y = pageH * pageIdx + cellY. Optionally
//   resolves cell occupant and updates drag-ghost widget. Does NOT send C2S or stamp cells.
//
// ABI (SEALED 2026-07-29): EAX=item*, ESI=window*, stdcall outX*,outY*,outOccupant*, ret 0x0c, AL 0/1
// Outs are PAGE-LOCAL; CanPlace uses absY = pageH*(i8)pageIdx + cellY (pageIdx @ window+0x564).
// Footprint extents: item+0x4a0 / +0x4a4. Page dims: typeHost+0x18/+0x1c via FUN_0085f1f0/f1d0.
// Decompiler stack noise (extraout_*, 0x85f266) is NOT authoritative — see dual A/B.
//
// READABILITY (CF): ifx17, returnx8, goto x1; callees CanPlace/FUN_0085f1d0/f1f0/0085e820/00413b30

/*
 * Behavioral notes:
 * - Dual-sealed 2026-07-29 for ABI, page-local outs, CanPlace absY, half-cell pitch constants.
 * - Remaining vtable rect map and runtime screen-scale globals are open.
...
uint32_t /* width from decompiler */ FUN_0085f220(int param_1,int param_2)
