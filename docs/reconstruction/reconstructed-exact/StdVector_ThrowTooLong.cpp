...
/* StdVector_ThrowTooLong — shared vector size-overflow throw (noreturn).
   ...
   Algorithm:
     ...
     3. FUN_00401a40(&exception_object, &message) — typed exception ctor
        (stores vtbl PTR_FUN_009c7634).
     4. _CxxThrowException(&exception_object, &DAT_00acc388).

   Returns: never.

   Callers: many vector grow/insert paths when requested element count would
   exceed max size (e.g. FUN_004507a0 overflow arm). Sibling throw sites with
   the same plate string exist at other VAs; this unit is the widely shared one. */
...
void __cdecl StdVector_ThrowTooLong(void)
{
