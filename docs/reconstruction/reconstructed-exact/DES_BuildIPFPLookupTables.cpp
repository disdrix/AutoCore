...
/*
 * Behavioral notes:
 * - Builds one 0x800-byte nibble-oriented block-permutation LUT from a 64-entry
 *   1-based FIPS perm vector (IP or FP).
 * - Sole caller DES_InitializeCipher:
 *     DES_BuildIPFPLookupTables(&DAT_00d16ef0, &DAT_00afca50); // IP
 *     DES_BuildIPFPLookupTables(&DAT_00d12690, &DAT_00afca90); // FP
 * - Consumer: DES_PermuteBlock (aa_0071bd40) ORs precomputed 8-byte rows.
 * - Leaf. Masks DAT_00afcd68 / DAT_00afcd88. Control flow preserved from Ghidra.
 */

void DES_BuildIPFPLookupTables(undefined4 *param_1, int param_2)
{
  int iVar1;
  byte *pbVar2;
  uint uVar3;
