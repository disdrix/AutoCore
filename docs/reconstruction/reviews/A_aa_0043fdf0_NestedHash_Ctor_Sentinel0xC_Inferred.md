# Review A (reconstruction fidelity): `aa_0043fdf0` NestedHash_Ctor_Sentinel0xC_Inferred

| Field | Value |
|---|---|
| **Stable ID** | `aa_0043fdf0` |
| **VA** | `0x0043fdf0`–`0x0043fe51` (**98 B**) |
| **Canonical name** | `NestedHash_Ctor_Sentinel0xC_Inferred` |
| **Ghidra name** | `FUN_0043fdf0` |
| **Review date** | `2026-07-29` (W34-F dual seal) |
| **Reviewer role** | Reconstruction fidelity (OWN-ONLY W34-F) |
| **Counterpart** | `reviews/B_aa_0043fdf0_NestedHash_Ctor_Sentinel0xC_Inferred.md` |
| **System** | shared nested hash/map shell |
| **Evidence pass** | Live Ghidra `batch_decompile` + `read_memory` (full 98 B) + `analyze_function_complete` xrefs + caller decompiles |
| **Verdict** | **accept-with-gaps** |

**Tools:** decompile + read_memory only (no `disassemble_bytes`). No Launcher. No parent ledger.

---

## 1. Purpose

Placement constructor for a **nested hash/map shell** (span 0x28):

1. SEH frame `LAB_009bc89b`.
2. `FUN_004933f0()` / `StdList_AllocSentinel_0xC` → circular sentinel `new(0x0C)` → store at **`this+0x08`**.
3. Zero **`this+0x0c`**.
4. `FUN_004646e0(this+0x10, 9, &sentinel_copy)` — 9-bucket pointer vector filled with sentinel.
5. **`this+0x20 = 1`**, **`this+0x24 = 1`**; return `this`.

Three hosts place this shell at different outer offsets.

---

## 2. Inspected artifacts / evidence

| Artifact | Path / tool |
|---|---|
| Raw (+ W34-F append) | `docs/reconstruction/raw/aa_0043fdf0_FUN_0043fdf0.md` |
| Annotated | `docs/reconstruction/raw/aa_0043fdf0_FUN_0043fdf0.annotated.md` |
| Clean | `docs/reconstruction/reconstructed-exact/NestedHash_Ctor_Sentinel0xC_Inferred.cpp` |
| Clean twin | `docs/reconstruction/reconstructed-exact/FUN_0043fdf0.cpp` |
| Function record | `docs/reconstruction/functions/aa_0043fdf0_FUN_0043fdf0.md` |
| Named record | `docs/reconstruction/functions/aa_0043fdf0_NestedHash_Ctor_Sentinel0xC_Inferred.md` |
| Live | decompile ≡ raw CF; full 98 B hex; xrefs `0074e3ba`, `0096eeea`, `00996c02` |
| Context | W32-D Ass NestedHash twin; W32-P StdList_AllocSentinel_0xC; W33-B PtrVecShell_InitFill; W33-R 0x40 host note |

---

## 3. Signature (sealed)

```c
// stack(self*); RET 4; returns self in EAX
NestedHash_Sentinel0xC* NestedHash_Ctor_Sentinel0xC_Inferred(NestedHash_Sentinel0xC* self /*stack*/);
```

| Slot | Source | Conf |
|---|---|---|
| self | Stack `[esp+0x18]` → ESI after SEH+pushes | **High** |
| return | `MOV EAX,ESI` | **High** |
| cleanup | `ADD ESP,0x10; RET 4` | **High** |
| ECX-thiscall | **No** | **High** |

Decompiler `undefined FUN_0043fdf0(void)` is incomplete — do not trust as full ABI.

---

## 4. Control flow (clean ≡ raw ≡ bytes)

```
self = stack_arg
sentinel = FUN_004933f0()          // new(0xC) circular
self[+8] = sentinel
self[+0xc] = 0
FUN_004646e0(self+0x10, 9, &sentinel)
self[+0x20] = 1
self[+0x24] = 1
return self
```

| Stage | Match | Conf |
|---|---|---|
| sentinel @ +8 | **Yes** | **High** |
| size0 @ +0xc | **Yes** | **High** |
| bucket vec cap 9 | **Yes** | **High** |
| flags 1/1 | **Yes** | **High** |
| body 98 B / RET 4 | **Yes** | **High** |
| Three caller sites | **Yes** | **High** |
| Product class English | shape only | **Inferred** |

Full hex: see raw W34-F append.

---

## 5. Callers (sealed offsets)

| Caller | Call site | Nested base |
|---|---|---|
| `FUN_0074e380` | `0x0074e3ba` | `param_1 + 5` (float*) → **+0x14** in 0x40 outer |
| `FUN_0096eec0` | `0x0096eeea` | object base (`param_1`) |
| `FUN_00996bc0` | `0x00996c02` | `param_1 + 10` (dword*) → **+0x28** |

---

## 6. Gaps / open

1. Product/PDB type name across three hosts.
2. Meaning of unwritten `+0x00/+0x04` and flags `+0x20/+0x24`.
3. Why these hosts use 0x0C sentinel vs Ass 0x28 / sibling 0x30.
4. Runtime / bit-exact / differential.

**Verdict:** **accept-with-gaps**
