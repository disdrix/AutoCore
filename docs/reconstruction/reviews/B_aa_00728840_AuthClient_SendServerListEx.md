# Review B (skeptical / adversarial): `AuthClient_SendServerListEx` @ `0x00728840`
...
| **Date** | `2026-07-23` |
| **Counterpart** | `reviews/A_aa_00728840_AuthClient_SendServerListEx.md` |
...
| # | Claim | Attack |
|---|---|---|
| 1 | Fetches list locally | **Falsified — outbound request only** |
...
|---|---|---|
| Outbound string "ServerListEx" | High | Wrong → protocol/UI mismatch risk |
| Uses send-scope logger 00727c00 not inbound 00727b70 | High | Wrong → protocol/UI mismatch risk |
| Called with (authClient, sessionA, sessionB) from RecvLoginOk | High | Wrong → protocol/UI mismatch risk |
...
```
SendServerListEx(auth, sessionLo, sessionHi): request extended server list
```
