# Review A (reconstruction fidelity): `aa_004438c0` NestedHashBag_Ctor_Sentinel0x30_Inferred

| Field | Value |
|---|---|
| **Stable ID** | `aa_004438c0` |
| **VA** | `0x004438c0`–`0x00443921` (**98 B**) |
| **Canonical name** | `NestedHashBag_Ctor_Sentinel0x30_Inferred` |
| **Ghidra name** | `FUN_004438c0` |
| **Review date** | `2026-07-29` (W34-G dual seal) |
| **Reviewer role** | Reconstruction fidelity (OWN-ONLY W34-G) |
| **Counterpart** | `reviews/B_aa_004438c0_NestedHashBag_Ctor_Sentinel0x30_Inferred.md` |
| **System** | containers / NestedHashBag shell (sentinel 0x30) |
| **Evidence pass** | Live Ghidra `batch_decompile` / `decompile_function` + `read_memory` (full 98 B) + `analyze_function_complete` xrefs |
| **Verdict** | **accept-with-gaps** |

**Tools:** decompile + read_memory only (no `disassemble_bytes`). No Launcher.

---

## 1. Purpose

Placement constructor for NestedHash/HeadBag bag:

1. SEH frame `LAB_009bed5b`.
2. `FUN_005b43b0()` → circular sentinel `new(0x30)` → **`this+0x08`**.
3. Zero **`this+0x0c`**.
4. `FUN_004646e0(this+0x10, 9, &sentinel_copy)` — 9-bucket pointer vector filled with sentinel.
5. **`this+0x20 = 1`**, **`this+0x24 = 1`**; return `this`; **`RET 4`**.

Sole caller: `FUN_00754ee0` embeds at **host +0x44**.

---

## 2. Inspected artifacts / evidence

| Artifact | Path / tool |
|---|---|
| Raw | `docs/reconstruction/raw/aa_004438c0_FUN_004438c0.md` (+ W34-G append) |
| Annotated | `docs/reconstruction/raw/aa_004438c0_FUN_004438c0.annotated.md` |
| Clean | `docs/reconstruction/reconstructed-exact/NestedHashBag_Ctor_Sentinel0x30_Inferred.cpp` |
| Clean twin | `docs/reconstruction/reconstructed-exact/FUN_004438c0.cpp` |
| Function record | `docs/reconstruction/functions/aa_004438c0_FUN_004438c0.md` |
| Named record | `docs/reconstruction/functions/aa_004438c0_NestedHashBag_Ctor_Sentinel0x30_Inferred.md` |
| Live | decompile ≡ raw CF; full 98 B hex; sole xref `00754f3e` |
| Context | W32-D Ass NestedHash; W33-B PtrVecShell_InitFill; W30-O uses same 0x30 sentinel factory |

---

## 3. Signature (sealed)

```c
// stack(self*); RET 4; returns self in EAX
NestedHashBag_Sentinel0x30* NestedHashBag_Ctor_Sentinel0x30_Inferred(
    NestedHashBag_Sentinel0x30* self /*stack*/);
```

| Slot | Source | Conf |
|---|---|---|
| self | Stack `[esp+0x18]` → ESI after SEH+pushes | **High** |
| return | `MOV EAX,ESI` | **High** |
| cleanup | `ADD ESP,0x10; RET 4` | **High** |
| ECX-thiscall | **No** | **High** |

Decompiler `undefined FUN_004438c0(void)` is incomplete — do not trust as full ABI.

---

## 4. Control flow (clean ≡ raw ≡ bytes)

```
self = stack_arg
sentinel = FUN_005b43b0()   // new(0x30) circular
self[+8] = sentinel
self[+0xc] = 0
FUN_004646e0(self+0x10, 9, &sentinel)
self[+0x20] = 1
self[+0x24] = 1
return self
```

| Stage | Match | Conf |
|---|---|---|
| SEH install | decompile ≡ bytes | High |
| Sentinel + stores | decompile ≡ bytes | High |
| Fill n=9 | decompile ≡ bytes (`6a 09`) | High |
| Flags + return | decompile ≡ bytes | High |

---

## 5. Gaps

1. Product English for host class (`FUN_00754ee0` / vtbl `00a9f584`).  
2. Unwritten `+0x00/+0x04`; flag meaning `+0x20/+0x24`.  
3. Unowned full product contract of `FUN_005b43b0`.  
4. Runtime / bit-exact / differential — open.

---

## 6. Verdict

Fidelity pass seals ABI, layout, CF, sole caller, and 0x30 sentinel differentiator. Residual product plates → **accept-with-gaps**.
