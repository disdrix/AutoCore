# System map: Client input â†’ vehicle drive control axes

| Field | Value |
|---|---|
| System | Local-player **user input** â†’ vehicle entity drive axes (`+0x614` / `+0x618` / `+0x61c`) â†’ controller push |
| Priority | **High** â€” user-facing input/commands, then movement. **Not** physics-first. |
...
* Per-tick local client mapping of **held ActionMap actions** (and related analog device path) onto the **possessed vehicle entityâ€™s drive axes**.
* The three entity slots that encode player drive intent:
  * `entity+0x614` â€” longitudinal / throttle-brake axis (f32)
  * `entity+0x618` â€” steer axis (f32)
  * `entity+0x61c` â€” handbrake / sharp byte (u8)
...
* Secondary behaviors co-located in the same tick (camera look offsets, terrain cast / airborne impulse, quick-bar skills, secondary weapons) â€” documented as **related but secondary**.

**Out of scope (indexed only)**

* Havok `VehicleAction_applyAction` stage-1 steer ramp, analog deadzone curve, `hkDefaultSteering_update` wheel angles â€” see physics verified notes.
* Network pose / ghost apply (`Vehicle_setDrivingInputs`, `FUN_0053eec0`) â€” related **movement** for remote vehicles; **not** the local input producer. See `docs/MOTION_CLIENT_RE.md`.
* NPC / AI axis generation (`MoveToTarget3DPoint` @ `0x004fc650`) â€” same entity slots, different producer.
...
|--------:|------------------------|------|
| `0x009223b0` | `Client_Input_DriveControlTick` | **Primary.** Local player drive-control tick: sample binds â†’ set entity axes â†’ optional push. |
...
| `0x004f5620` | `VehicleEntity_SetSteerInput` | Gated write of steer â†’ `entity+0x618`. |
| `0x004f5650` | `VehicleEntity_SetLongitudinalInput` | Gated write of longitudinal â†’ `entity+0x614`. |
| `0x004f3620` | `VehicleEntity_SetHandbrake` | Write handbrake â†’ `entity+0x61c` (**no** `0xC7` gate documented on this setter). |
| `0x004fbc10` | `VehicleEntity_PushDriveAxesToController` | Bridge: entity thr/handbrake â†’ driver-input controller (steer **not** copied here). |

**Other producers of the same slots (not this systemâ€™s entry, but share state):**

| Address | Role |
|--------:|------|
| `0x004fc650` | AI / move-to-target axis generator |
| `0x00504c70` | `Vehicle_setDrivingInputs` â€” network/ghost path (axes + pose) |
...
param_1  â€” client / player controller object (VOGClient-related; exact type name uncertain)
param_2  â€” vehicle entity pointer, or 0 if none
```
...
High-level per-tick sequence for the **local driving** path (digital ActionMap name-3 path; analog path is a parallel producer â€” see Secondary):

```
1. Client_Input_DriveControlTick(controller, vehicle)
2. Early exit if UI blocks input
   â€” DAT_00d1b6d8[0x7f] vtbl+0x3c (exact object type uncertain; blocks drive sampling)
3. If vehicle present and vehicle+0x101 == 0 (drive-enabled):
   a. Sample held ActionMap flags (name-3 DAT_* action structs)
   b. Longitudinal:
        Accelerate held (DAT_00d1bc26) â†’ SetLongitudinalInput(vehicle, -1.0f)
        Reverse held     (DAT_00d1bc5a) â†’ SetLongitudinalInput(vehicle, +1.0f)
        (neither / release path â†’ typically 0; exact clear timing may share tick logic)
   c. Steer (**exclusive priority**, not additive):
        1. hard-R  (DAT_00d1bcc2) â†’ SetSteerInput(vehicle, -1.0f)   // 0xbf800000
        2. hard-L  (DAT_00d1bc8e) â†’ SetSteerInput(vehicle, +1.0f)   // 0x3f800000
        3. soft-L  (DAT_00d1bd2a) â†’ SetSteerInput(vehicle, -0.5f)   // 0xbf000000  (UF-006)
        4. soft-R  (DAT_00d1bcf6) â†’ SetSteerInput(vehicle, +0.5f)   // 0x3f000000
        5. none + no analog â†’ SetSteerInput(0)
   d. Handbrake:
        Brake held (DAT_00d1bd5e) â†’ SetHandbrake(vehicle, 1)
        release clears (SetHandbrake 0) â€” same tick family
...
5. Secondary same-tick work (may interleave; not all gated on vehicle+0x101 the same way):
   â€” analog device path (DAT_00d1b618 / DAT_00d1b620) â€” see Soft/analog residual below
   â€” camera / look offsets (local_d* stack locals â€” **not** steer axis; see Camera residual)
   â€” terrain cast CVOGMap_CastTerrainHeight (airborne impulse helper)
   â€” quick-bar skill activation
   â€” Input_TryFireSecondaryWeapons
```

### Camera look-offset residual (SECTION 1 / 3 / 5â€“7 / 15) â€” 2026-07-29 (strengthen)

**Locals vs steer (High / sealed structural):** drive steer is only `SetSteerInput` â†’ `entity+0x618`. Stack `local_d*` are camera/look state co-located in the same tick; SECTION 7 bind branches may update **both** independently. Separation axes: storage / gate (`+0x101` on setters only) / value domain (steer Â±1/Â±0.5 vs look `local_d4` Â±0.03/Â±0.2) / consumer (`FUN_0091a5f0`+SECTION 15 vs physics on `+0x618`). Soft look Â±0.2 â‰  soft steer Â±0.5 (UF-006).
...
|-----|----:|-------------------------------------|
| `DAT_00aaa8fc` | 0.03 | hard L/R â†’ `local_d4` Â± |
| `DAT_00a0f70c` | 0.2 | soft L/R â†’ `local_d4` Â± |
| `DAT_00aaa878` | 0.004 | alternate-mode `local_d4` step |
| `DAT_00aaa688` | 5.0 | hard-R â†’ add to `local_c8` |
| `DAT_00aaacc8` | 11.0 | default look step `local_a4` |
| `DAT_00aaacc4` | âˆ’5.0 | reverse special look step |
...
|------|------------|-----------|----:|-----------------|---------|
| `DAT_00d1bd2a` | Soft L | `0xbf000000` | âˆ’0.5 | `0x00922c48` | **High** body |
| `DAT_00d1bcf6` | Soft R | `0x3f000000` | +0.5 | `0x00922cf8` | **High** body |
| `DAT_00d1bc8e` | Hard L | `0x3f800000` | +1.0 | decompile | **High** |
| `DAT_00d1bcc2` | Hard R | `0xbf800000` | âˆ’1.0 | decompile | **High** |

**UF-006 (open):** SoftL body = âˆ’0.5 while HardL body = +1 â€” **opposite signs** if both labels mean physical left. Display strings â€œSteer Left Softâ€ / â€œSteer Right Softâ€ (`Client_InitActionDisplayNames` `+0x34d`/`+0x381`) do not alone prove DAT identity. **Do not invent a fix** (no clean/value flip without runtime bind hold). CX-001 remains open.
...
| Sample thr | `DAT_00d1b620(7) âˆ’ DAT_00d1b620(6)` | High CF |
| Sample steer | `DAT_00d1b620(9) âˆ’ DAT_00d1b620(10)` | High CF |
...
| Deadzone/scale table | Partial â€” continuous mapping not fully expanded | Mediumâ€“Low |
...
Client_Input_DriveControlTick                 0x009223b0
  â”‚
  â”œâ”€[UI block?]â”€â”€â–º early return
  â”‚
  â”œâ”€ VehicleEntity_SetLongitudinalInput       0x004f5650  â†’ entity+0x614
  â”œâ”€ VehicleEntity_SetSteerInput              0x004f5620  â†’ entity+0x618
  â”œâ”€ VehicleEntity_SetHandbrake               0x004f3620  â†’ entity+0x61c
  â”‚
  â”œâ”€ VehicleEntity_PushDriveAxesToController  0x004fbc10
  â”‚     â””â”€ writes driverInput thr (+0x20), handbrake (+0x24)
  â”‚        (ctrl = handle[entity+0x1a0]+8)
  â”‚
  â”œâ”€ (secondary) analog sampling              DAT_00d1b618 / DAT_00d1b620
  â”œâ”€ (secondary) camera/look offset locals
  â”œâ”€ (secondary) CVOGMap_CastTerrainHeight    0x004cfe60  (xref from ~0x9230c7)
  â”œâ”€ (secondary) quick-bar skill activation
  â””â”€ (secondary) Input_TryFireSecondaryWeapons

â”€â”€â”€ Downstream (indexed; not re-verified in this map) â”€â”€â”€

entity+0x618
  â†’ VehicleAction_applyAction                 0x00598650  stage-1 ramp â†’ VA+0x24
  â†’ mirror DI+0x1c
  â†’ FUN_005fdf20 / calcStatus                 â†’ DI+0x14 filtered steer
  â†’ hkDefaultSteering_update                  0x0064f840

entity+0x614  (via Push â†’ DI+0x20) + entity+0x61c (via Push â†’ DI+0x24)
  â†’ transmission / wheel torque / brake consumers under applyAction family
```

**Primary important unit for this goal:** `aa_004f5620` / `VehicleEntity_SetSteerInput` â€” pure **gate + store** of the steer command the rest of the movement stack consumes.
...
|--------|-----------|-------------|
| ActionMap held: Accelerate `DAT_00d1bc26` | map to `-1.0f` | `SetLongitudinalInput` â†’ `entity+0x614` |
| ActionMap held: Reverse `DAT_00d1bc5a` | map to `+1.0f` | `SetLongitudinalInput` â†’ `entity+0x614` |
| ActionMap held: SteerLeft `DAT_00d1bc8e` | map to `+1.0f` | `SetSteerInput` â†’ `entity+0x618` |
| ActionMap held: SteerRight `DAT_00d1bcc2` | map to `-1.0f` | `SetSteerInput` â†’ `entity+0x618` |
| Soft L `DAT_00d1bd2a` | map to `-0.5f` (`0xbf000000`; image `push` @ `0x00922c48`) | `SetSteerInput` â†’ `entity+0x618` â€” **UF-006 label open** |
| Soft R `DAT_00d1bcf6` | map to `+0.5f` (`0x3f000000`; image `push` @ `0x00922cf8`) | `SetSteerInput` â†’ `entity+0x618` |
| Brake `DAT_00d1bd5e` | map to `1` / release `0` | `SetHandbrake` â†’ `entity+0x61c` |
| Analog device path | continuous axes: thr ch7âˆ’ch6, steer ch9âˆ’ch10; clamp thr; same setters when `+0x101==0` | same entity slots (`DAT_00d1b618` / `DAT_00d1b620`) |
...
| `entity+0x618` | **not** via Push | read later by `applyAction` stage-1 into `VA+0x24` â†’ `DI+0x1c` â†’ filtered `DI+0x14` |
  ...
  ActionMap DAT_* flags     DAT_00d1b618/620 path
     ...
     SetLongitudinal / SetSteer / SetHandbrake
         ...
         entity +0x614 / +0x618 / +0x61c
              â”‚              â”‚
              â”‚              â””â”€â”€â–º applyAction (steer ramp; next physics substeps)
              â–¼
...
|-------|--------------|--------------------------|-----------|
| Longitudinal command | Vehicle entity `+0x614` | `SetLongitudinalInput` from DriveControlTick (and other producers) | `PushDriveAxes` â†’ controller; physics thr path |
...
| Handbrake / sharp | Vehicle entity `+0x61c` | `SetHandbrake` (DriveControlTick + AI/UI packs); **also** direct stores from MoveToTarget / SetDriveAxes | `PushDriveAxes` â†’ `ctrl+0x24`; `calcWheelTorque` (rear torque Ã—0.5) |
...
| Gate closed (`wobj+0xb4 & 0xC7`) | SetSteer / SetLongitudinal called | **No write** â€” previous axis retained |
| `entity+0x101 != 0` | Tick would accelerate / push | Longitudinal accelerate path skipped; Push no-op |
| UI block active | Tick entry | Early exit â€” no axis sampling (drive path) |
...
| Terrain height query | Secondary path in same tick | `CVOGMap_CastTerrainHeight` â€” airborne impulse helper, not wheel collision |
...
| Network outbound | **Not established in this map** | Local axis â†’ server replication (if any) not traced here |
...
| ActionMap action records (`DAT_00d1bc*`) | Mediumâ€“High | Held-action flags; â€œname-3â€ family from plate comments |
...
| UI block object via `DAT_00d1b6d8[0x7f]` | Lowâ€“Medium | vtbl+0x3c call; type not named |
...
|--------:|------------|------|
| `0x009223b0` | `Client_Input_DriveControlTick` | Input â†’ axes tick (dual A/B reviews) |
...
| `0x004fbc10` | `VehicleEntity_PushDriveAxesToController` | Entity â†’ controller thr/HB |
| `0x004fc650` | `CVOGVehicle_MoveToTarget3DPoint` | AI path thr/steer/HB (UF-007 gate) |
| `0x007f9110` | `Client_ActionMap_Init` | Zero tables â†’ names â†’ default DIKs |
...
| `0x00911030` | `Client_Input_OnKeyDown_MatchAction` | WM key-down â†’ held/edge flags |
...
| Offset | Type | Meaning |
|-------:|------|---------|
| `entity+0x101` | u8 | Drive disabled if â‰  0 |
| `entity+0x109` | u8 | Hard-stop flag (Push forces thr=0, HB=1) â€” Push path, not ActionMap |
| `entity+0x1a0` | ptr | Handle block; `+8` â†’ driver-input controller |
...
### 1. `Client_Input_DriveControlTick` @ `0x009223b0` â€” **system root**

* Args: controller object, vehicle entity (or 0).
* Samples ActionMap held actions â†’ digital axis magnitudes.
...
### 2. `VehicleEntity_SetSteerInput` @ `0x004f5620` â€” **primary important unit**
...
* Retail accelerate = **âˆ’1**, reverse = **+1**.

### 4. `VehicleEntity_SetHandbrake` @ `0x004f3620`

* Writes `entity+0x61c`; **no** `0xC7` gate (dual residual 2026-07-29 sealed; live â‰¡ raw).
...
|-------|--------|----------|
| DriveControlTick signature, param roles, ActionMapâ†’setter mapping, soft Â±0.5, brake, Push at end if `+0x101==0`, secondary paths (analog, camera, terrain, skills, weapons), UI early-out | Fresh Ghidra decompile of `Client_Input_DriveControlTick` @ `0x009223b0` (this pass) | **High** (input layer) |
...
| Downstream `entity+0x618` â†’ VA+0x24 â†’ DI+0x1c â†’ DI+0x14 â†’ `0x64f840` | `fn_steering_input_feed.md` | **Verified** (physics; indexed here) |
...
| Soft vs hard exclusive priority hard-Râ†’hard-Lâ†’soft-Lâ†’soft-Râ†’clear | Fresh decompile SECTION 7 if/else chain | **High** |
| UF-006 SoftL sign vs HardL | Body SoftL=âˆ’0.5 HardL=+1; labels unproven physical | **Open residual** (do not invent fix) |
| Analog continuous â†’ same setters when drive-enabled | SECTION 3 decompile | **High** CF; scale details Medium |
| Camera look locals â‰  steer axis; SECTION 7 interleave | Clean SECTION 1/3/5â€“7/15 + dual camera refresh; look steps `read_memory` 0.03/0.2/5/11 | **High** CF (locals vs steer); component names open |
...
* `docs/MOTION_CLIENT_RE.md` (network pose â€” related movement, not primary input)
...
| Area | Level | Notes |
|------|-------|-------|
| Digital ActionMap â†’ axis magnitudes and signs | **High** | Fresh decompile + matching setter plates |
...
| Analog device path | **Mediumâ€“Low** | Globals known; continuous mapping not fully expanded |
| UI block object type / all early-out conditions | **Mediumâ€“Low** | One known probe; may be incomplete |
| Soft vs hard steer priority (exclusive order) | **High** | Sealed if/else: hard-R â†’ hard-L â†’ soft-L â†’ soft-R â†’ clear |
| Soft L/R physical polarity vs hard (UF-006) | **Tentative** | Body values High; labels open â€” runtime needed |
...
| Look-vector component names / full mode tables | **Lowâ€“Medium** | Secondary; open residual |
...
3. ~~Soft steer vs hard steer priority~~ â€” **sealed exclusive:** hard-R â†’ hard-L â†’ soft-L â†’ soft-R â†’ clear (not additive).
4. Is longitudinal axis forced to `0` every tick when neither thr/rev is held, or only on transition?
5. Full analog deadzone/scale: `DAT_00d1b618` / `DAT_00d1b620` continuous â†’ setters (CF sealed; curve table partial).
5b. UF-006: does SoftL physical match HardL under real binds? (body SoftL=âˆ’0.5 vs HardL=+1 residual)
...
10. ~~Camera look locals vs steer~~ â€” **sealed structural:** `local_d*` â‰  `+0x618`; residual open only for component names / full combat look tables / `FUN_0091a5f0` deep semantics.
...
| UI early-out completeness | â€œStuck axesâ€ when menus open | Enumerate all early returns |
| Live: hold Accelerate â†’ confirm `+0x614 == -1` then Push thr | End-to-end smoke | CE/hook on entity slots during local drive |
| Soft body magnitudes âˆ’0.5 / +0.5 | **Sealed** (decompile + image push) | â€” residual closed for hex |
| UF-006 SoftL/SoftR **physical** direction vs hard | Label polarity residual | Runtime hold Soft L/R bind â†’ CE `entity+0x618` |

---

## Next recommended work

1. **Promote** a verified function note for `Client_Input_DriveControlTick` (`0x009223b0`) with full ordered branch list (digital, analog, secondary) and all early returns â€” same quality bar as `fn_004f5620_setSteerInput.md`.
...
4. **Leave physics ramps alone** until input axes are correct â€” stage-1 / DI filter already verified; wrong input signs are the higher-risk user-facing bug.
...
User-facing **commands** (what the player holds) and the **movement axes** those commands become are the first mile of vehicle control. Physics ramps and steering falloff only scale what this system writes. Network pose (`MOTION_CLIENT_RE.md`) moves remote bodies; it does not replace local ActionMap â†’ entity axes. This map therefore stops at entity/controller handoff and **indexes** the rest.

## Mega residual duals (2026-08-05)

Nested undualed callees of dualed drive/input managers (axis park, ActionMap slot, keymap.ini, mission-UI/dialog thr/steer park).

16 dualed units from mega residual partition (MEGA-001..140). Parent merge: dual unique 2428→2567. Runtime Confirmed: none. Terminal: false.

| Address | Symbol | Notes |
|--------:|--------|-------|
| `0x005d73a0` | `CVOGHBAIDriver_AxisPark_Inferred` | mega residual dual 2026-08-05 (accept-with-gaps) |
| `0x00636ba0` | `CVOGHBMoveVehicle_OnEnd` | mega residual dual 2026-08-05 (accept-with-gaps) |
| `0x007f6db0` | `ActionMap_FindSlotByKeyMod_Inferred` | mega residual dual 2026-08-05 (accept-with-gaps) |
| `0x007f74a0` | `ActionMap_ClearAllSlotBindFields_Inferred` | mega residual dual 2026-08-05 (accept-with-gaps) |
| `0x0092f090` | `MissionUI_ParkDriveAxes_Inferred` | mega residual dual 2026-08-05 (accept-with-gaps) |
| `0x0092f710` | `Client_LoadKeymapIni` | mega residual dual 2026-08-05 (accept-with-gaps) |
| `0x009373e0` | `Client_DialogGate_OpenIndex10_ParkDriveAxes_Inferred` | mega residual dual 2026-08-05 (accept-with-gaps) |
| `0x005fdf20` | `hkDefaultAnalogDriverInput_FilterSteer_Inferred` | mega residual dual 2026-08-05 (accept-with-gaps) |
| `0x00402850` | `StdTree_EraseAndRebalance_Isnil29_RetC_Inferred` | mega residual dual 2026-08-05 (accept-with-gaps) |
| `0x00401c30` | `StdVector_DwordSize_EaxVec_Inferred` | mega residual dual 2026-08-05 (accept) |
| `0x005cc560` | `CVOGHBAI_DecEntityA4SecondaryCounter_Inferred` | mega residual dual 2026-08-05 (accept-with-gaps) |
| `0x00790020` | `Client_Input_KeyDown_UiGateAndForward_Inferred` | mega residual dual 2026-08-05 (accept-with-gaps) |
| `0x0090d390` | `Client_Input_FindFirstActiveUiWindowTable5_Inferred` | mega residual dual 2026-08-05 (accept-with-gaps) |
| `0x0090dab0` | `Client_Input_DismissUiWindowTable5_Inferred` | mega residual dual 2026-08-05 (accept-with-gaps) |
| `0x00914c20` | `Client_Vehicle_StopHbAndZeroLongitudinal_Inferred` | mega residual dual 2026-08-05 (accept-with-gaps) |
| `0x00580ed0` | `TtPhantom_castRay` | mega residual dual 2026-08-05 (accept-with-gaps) |
