...
## R13-038 re-verify (2026-08-05) — OWN-ONLY dual seal

| Field | Value |
|---|---|
| **Tools** | `decompile_function` + `analyze_function_complete` + `get_function_callers` + `get_function_xrefs` + `get_assembly_context` + `read_memory` + `get_function_by_address` |
| **No** | `disassemble_bytes`; Launcher; parent ledgers |
| **Partition** | R13-038 (`WAVE_2026-08-05_r13_residual_partition_map.md`) |
| **Dual start** | 2686 |
| **Terminal** | false |
...
| Item | Value |
|---|---|
| Body | `0x0054cc10`–`0x0054d43f` inclusive (**2096 B** / `0x830`); terminal bare **`C3`** |
| Next function | `0x0054d440` |
| Classification | **leaf** (0 callees) |
| ABI | **thiscall / ECX=this**; 0 stack args; bare **RET**; void (EAX not set for return) |
| Semantics | Zero dword/float slots **`this+0x28` … `this+0x534`** inclusive (**324** / `0x144` dwords; span `0x510` B) via `XORPS`/`MOVSS`/`MOV` |
| Code xrefs | **1** UNCONDITIONAL_CALL — `0x0054a2d1` in `FUN_0054a290` (`SkillDef_InstallLoadedRecord`) |
| Call site | `LEA ECX,[ESP+0x1a0]` then `CALL 0x0054cc10` — stack skill-staging buffer |
| Post-call (caller) | `REP STOSD` **0x14e** dwords on same `[ESP+0x1a0]` (full `0x538`-byte staging clear) |
| DATA xrefs | **0** |
| Live decompile ≡ raw CF | **Yes** (all stores = 0; return) |
| Entry bytes | `0F 57 C0  8B C1  F3 0F 11 40 2C  F3 0F 11 40 30  33 C9  89 48 28 …` |
| Exit bytes @ `0x0054d438` | `F3 0F 11 80 34 05 00 00  C3` (`movss [eax+0x534],xmm0; ret`) |

### Naming (this pass)

`SkillStaging_ZeroPayloadFields_Inferred` — role Inferred from sole caller + field range; not Runtime Confirmed.

### Note on parent dual

Partition parent dual VA is **`0x0054cbb0`** (PossessCreature DefaultFactory, adjacent leaf). Semantic caller is **`0x0054a290`** skill-def install (not dualled in this OWN unit).
