# Annotated low-level: Client_UI_CreateNDUIDialogs_InstallHost_Inferred

| Field | Value |
|---|---|
| Stable ID | `aa_0093e7e0` |
| VA | `0x0093e7e0`–`0x0093ffa3` (**6084 B** / `0x17C4`) |
...
| Date | 2026-08-05 (MEGA-133 dual re-verify; prior scaffold 2026-07-23) |
| Verdict | **accept-with-gaps** |

## Machine-level notes (sealed)

- **ABI:** `__stdcall` — stack `ClientUiHost* host` loaded into **EBP**; epilogue **`RET 4`**. Not ECX-thiscall.
- **SEH:** `LAB_009bb29b` / FS:[0]; frame size `SUB ESP,0x470` then pushes.
- **Role:** bulk **CreateNDUIDialogs** host install — sole callee of `FUN_0094a580` after log `"createNDUIDialogs"`.
- **Timing:** `FUN_007a4400` copies phase plate into buffer and stamps QPC; `FUN_007a4390` dtor; end prints `(%f) %s\n` via `OutputDebugStringA`.
- **Cursor:** `host+0x1168` — load `i_g_2d_cursor.xml` via vtbl+0x28; finalize vtbl+0x180; rebind +0x88 from `*(host+0xde8)+0x2c`.
- **Install pattern:** `operator_new` → ctor → store host slot / global; many SEH state bytes.
- **Death respawn:** `host+0x1158` + XML `i_g_2d_btn_death_respawn.xml` + id `0x4e5d` — **not** the whole function purpose.
- **Post-pass:** 0x3a-slot vtbl+0x174; pair flags `dialog+0xc6=1`; `host+0x1100` forced 0; `FUN_00931570` + `"winpos.ini"`.
- Prefer assembly when decompiler omits args on vtbl calls (many CALL sites pass XML/ids in stack).
- Prefer `read_memory` for plate strings over inventing product dialog names.

## Rejected names

- `Combat_DeathRespawnButton` — single widget plate.
- `Named_CalleeOf_Mission_createNDUIDialogs_0093e7e0` — scaffold retired.

## Pseudocode (live decompile 2026-08-05 ≡ raw CF)

```c/* WARNING: Globals starting with '_' overlap smaller symbols at the same address */
...
## Host slot map (install stores; product English residual)

| Offset | Role (inferred from CF only) |
|---|---|
| `+0x9b8` | dialog* after `FUN_0085e590` / vtbl+0x300 |
| `+0xde8` | source for cursor rebind (+0x2c) |
| `+0xe04` | graphics/UI root (`+0xe890` passed into several ctors) |
| `+0xf38` / `+0xf3c` | title/action family (`FUN_00920790` / `FUN_00913600`) |
| `+0x1030`.. | dense dialog* table (0x3a entries for winpos / vtbl+0x174) |
| `+0x1100` | special: post-flag `+0xc6` forced **0** |
| `+0x1158` | death-respawn button dialog |
| `+0x1168` | cursor / CVOGCursor host object |
| `+0x3084` | CNDHash (`FUN_004063a0`) |
| `+0x309c`..`+0x30b0` | early NDUI dialog cluster |
| `DAT_00d09a38`..`48` | global dialog* installs |

## Cross-refs

| Kind | Value |
|---|---|
| Caller | `FUN_0094a580` @ `0x0094a649` only |
| Post helper | `FUN_00931570` (winpos.ini layout) — residual dual |
| Twin clean | `Client_UI_CreateNDUIDialogs_InstallHost_Inferred.cpp` |

## Open gaps

- Dialog class / product names per ctor.
- Vtbl method English.
- Runtime / bit-exact.