...
undefined4 __thiscall FUN_00754a10(uint param_1,char *param_2,int param_3)
  ...
  if (pHVar4 == (HFONT)0x0) {
    ...
    *(HFONT *)(unaff_ESI + 0xa4) = pHVar4;
    if (pHVar4 == (HFONT)0x0) {
      ...
      *(undefined4 *)(unaff_ESI + 0x38) = 0x100;
      if (0x100 < iVar2 * 2) {
        do {
      ...
      *(undefined4 *)(unaff_ESI + 0x3c) = 0x100;
...
| **Tools** | `batch_decompile` / `analyze_function_complete` + `read_memory` (full body) + callers/xrefs + factory caller `FUN_0073bc50` call-site bytes. **No** `disassemble_bytes`. |
| **Body** | `0x00754a10`–`0x00754edc` exclusive (**1228 B** / `0x4CC`; pad `CC` then `FUN_00754ee0` GlyphCache_PlacementCtor) |
...
|---|---|---|
| `this` (GlyphCache*) | **ESI** | stores to `ESI+0x18..+0xB4`; caller `MOV ESI,EDI` |
| style flags | **ECX** | entry `MOV EBX,ECX`; bits 0/1/2/3 → Bold/Italic/Shadow/Outline |
| face name | stack arg | `_snprintf` / `strncpy` source; caller `PUSH EDX` |
| height (lfHeight) | stack arg | `LOGFONT.lfHeight = param`; caller `PUSH EBX` |
| cleanup | **`RET 8`** | epilogue `ADD ESP,0x1D4; C2 08 00` |
| return | **EAX** | `0` success / `0xFFFFFFFF` fail (`XOR EAX,EAX` / `OR EAX,-1`) |

Decompiler marks `__thiscall` with style in ECX and `unaff_ESI` for the host — **ESI is the real this** (custom register this + ECX style).

### Semantics (sealed CF)

1. Optional one-shot `FUN_00754970` if `DAT_00d1a548 == 0`.
2. Build style tag string: `|Bold` / `|Italic` / `|Shadow` / `|Outline` bits; empty → `"Normal"`; else strip leading `|`.
3. Display name `"%s %i (%s)"` → `this+0x18`; store style at `this+0x34`.
4. Fill `LOGFONTA`: weight 400 or 700 (bold), italic bit, height, charset `1`, out-precision `8`, quality `5` (or `3` if height < `0xE`), face `strncpy` 0x20.
5. `CreateFontIndirectA` → `this+0xA4`; fail → log `"Failed CreateFont."` line `0x8E`.
6. `CreateCompatibleDC(0)` → `this+0xA8`; fail → formatted log line `0x93`.
7. `SetMapMode(MM_TEXT)`; select font; `GetTextMetricsW`; fail → log line `0x9C`.
8. Restore / `DeleteObject` temp font; recreate + select; publish metrics:
   - `+0x6C` quality; `+0x74` ascent; `+0x78` descent; `+0x80` external leading
   - `+0x88` first char; `+0x8A` last char; `+0x7C` height+extLead; `+0x70` height; `+0x84` max width
   - Shadow (`bit2`): height+1, maxw+2; Outline (`bit3`): height+2, maxw+3
9. Atlas power-of-two from 0x100, capped by `DAT_00d1f044+0x6C/0x70` (GfxDevice); `+0x38` atlasW, `+0x3C` atlasH, `+0x40` rows = atlasH/cellH; `+0xB0/+0xB4` cell copies.
10. `CreateDIBSection` 32bpp top-down (`biHeight = -cellH`) → `+0xA0` HBITMAP, bits at `+0xAC`.
11. Success: select DIB, white text, transparent bk, align 0; return **0**.
12. Fail paths: `vog_LogMessage("…\gfxFontImpl.cpp", line, 3, msg)` → return **0xFFFFFFFF**.

### Product evidence

- Source path string: `C:\vog\1_code\palantir\palantir\graphics\gfxFontImpl.cpp`
- Style tokens `|Bold` `|Italic` `|Shadow` `|Outline` `"Normal"`
- Error strings CreateFont / CreateCompatibleDC / GetTextMetrics / CreateDIBSection
- Host size `0xB8` matches `GlyphCache_PlacementCtor` (W35-H); sole factory `GlyphCache_GetOrCreate_ByFontKey` (W37-AE)

### Epilogue anchors

```text
0x00754a10: 6A FF 64 A1 … 81 EC C8 01 00 00   ; SEH + sub esp 0x1C8
            8B D9                              ; EBX = ECX (style)
…
success:    33 C0 … 81 C4 D4 01 00 00 C2 08 00  ; EAX=0; add esp 0x1D4; RET 8
fail:       83 C8 FF … C2 08 00                 ; EAX=-1; RET 8
```
...
| Dir | Target |
|---|---|
| Caller (1) | `FUN_0073bc50` @ `0x0073bd22`, `0x0073bd33` (requested face then `"Arial"` @ `0x00aa267c`) |
| Callees | `FUN_00754970`, `FUN_0076cec0`, `vog_LogMessage`, basic_string ops, Win32 GDI (`CreateFontIndirectA`, `CreateCompatibleDC`, `GetTextMetricsW`, `CreateDIBSection`, `SelectObject`, `DeleteObject`, `SetMapMode`, `SetTextColor`, `SetBkMode`, `SetTextAlign`, `GetLastError`), `_snprintf`, `strncpy` |

### Gaps

- Product English class/method plate beyond path `gfxFontImpl.cpp`.
- Full meaning of `DAT_00d1a548` / `FUN_00754970` one-shot (unowned).
- Exact GfxDevice fields at `DAT_00d1f044+0x6C/0x70` (cap only sealed).
