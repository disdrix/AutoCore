...
| Field | Value |
|---|---|
| **Tool** | Ghidra HTTP `decompile_function` + `read_memory` + callers/callees/xrefs |
| **Body** | `0x0060d630`–`0x0060d7e1` |
| **ABI** | `__thiscall`; `ret 0x0C` (`C2 0C 00`); AL 0/1 |
| **Caller** | sole `FUN_0060df70` @ `0x0060e061` (UseItem object-event path, events 9/10) |
| **Class** | `CVOGObjectiveRequirement_UseItem` (RTTI via vtable `PTR_FUN_009dfc1c` COL `0x00ab151c` → `.?AVCVOGObjectiveRequirement_UseItem@@`) |
| **String** | `0x009e00a0` `Failed to give item %d to %I64d, objective %d mission: %S(%d)` |
