# Annotated low-level: FUN_005be840

| Field | Value |
|---|---|
| Stable ID | `aa_005be840` |
| VA | `0x005be840` |
| System | unknown |
| Date | 2026-07-23 |

## Machine-level notes

- Source: raw capture for `aa_005be840`.
- Prefer assembly when decompiler conflicts.
- Recover types for still-generic parameters via callers/xrefs.
- Map DAT_* globals and FUN_* callees in follow-up waves.

## Pseudocode (annotated copy of raw)

```c
void __thiscall FUN_005be840(int *param_1,int param_2,int param_3,int param_4)

{
  int *piVar1;
  int *piVar2;
  int iVar3;
  undefined4 uVar4;
  int *piVar5;
  int iVar6;
  int *piVar7;
  int unaff_retaddr;
  float fStack_28;
  void *pvStack_24;
  float fStack_20;
  undefined4 uStack_1c;
  undefined4 uStack_18;
  undefined4 uStack_14;
  void *pvStack_10;
  void *pvStack_c;
  undefined1 *puStack_8;
  undefined4 uStack_4;
  
  uStack_4 = 0xffffffff;
  puStack_8 = &LAB_009a6c60;
  pvStack_c = ExceptionList;
  ExceptionList = &pvStack_c;
  param_1[0x34] = param_2;
  param_1[0x35] = param_3;
  if ((*(int *)(param_4 + 900) <= param_2) || (*(int *)(param_4 + 0x388) <= param_3)) {
    FUN_007a4480(0,"VOG_DEBUG_STOP");
  }
  piVar5 = param_1 + 0x36;
  for (iVar6 = 0x6000; iVar6 != 0; iVar6 = iVar6 + -1) {
    *piVar5 = 0;
    piVar5 = piVar5 + 1;
  }
  FUN_004ac660(param_1 + 0x36,param_2,param_3);
  iVar3 = (**(code **)(**(int **)(param_4 + 0x390) + 0xc))();
  iVar6 = *param_1;
  param_1[0x19] = iVar3;
  uVar4 = (**(code **)(**(int **)(param_4 + 0x390) + 100))();
  (**(code **)(iVar6 + 0x34))(uVar4);
  piVar5 = *(int **)(param_4 + 0x394);
  piVar7 = (int *)*piVar5;
  if ((piVar7 != (int *)0x0) && (piVar7[1] = piVar7[1] + 1, piVar7[1] == 1)) {
    (**(code **)(*piVar7 + 4))();
  }
  piVar2 = (int *)param_1[8];
  if (piVar2 != (int *)0x0) {
    piVar1 = piVar2 + 1;
    *piVar1 = *piVar1 + -1;
    if (*piVar1 == 0) {
      (**(code **)(*piVar2 + 8))();
    }
  }
  param_1[8] = (int)piVar7;
  *(char *)(param_1 + 9) = (char)piVar5[1];
  *(undefined1 *)((int)param_1 + 0x25) = *(undefined1 *)((int)piVar5 + 5);
  *(undefined1 *)((int)param_1 + 0x26) = *(undefined1 *)((int)piVar5 + 6);
  FUN_0096fdf0();
  param_1[0x2f] = param_1[0x2f] | 1;
  pvStack_24 = (void *)0x0;
  fStack_20 = 0.0;
  uStack_1c = 0;
  puStack_8 = (undefined1 *)0x0;
  uStack_18 = 0;
  uStack_14 = 0;
  FUN_007478c0(0x4d2);
  puStack_8 = (undefined1 *)0x1;
  iVar6 = FUN_00414c20(0x600,&fStack_28,0x20,0);
  puStack_8 = (undefined1 *)0xffffffff;
  uStack_18 = 0;
  uStack_14 = 0;
  if (pvStack_24 == (void *)0x0) {
    if (-1 < iVar6) {
      iVar6 = param_1[5];
      if ((*(int *)(iVar6 + 0x10) != 0) &&
         (piVar5 = (int *)FUN_00746520(*(undefined4 *)(iVar6 + 0x14),*(undefined4 *)(iVar6 + 0x18),0
                                      ), piVar5 != (int *)0x0)) {
        piVar7 = param_1 + 0x36;
        for (iVar6 = 0x6000; iVar6 != 0; iVar6 = iVar6 + -1) {
          *piVar5 = *piVar7;
          piVar7 = piVar7 + 1;
          piVar5 = piVar5 + 1;
        }
        if (*(int *)(param_1[5] + 0x10) != 0) {
          FUN_007464e0();
        }
        param_1[0x2a] = 0x200;
        param_1[0x2b] = 4;
        (**(code **)(*param_1 + 0x20))(*(undefined4 *)(param_4 + 0x390));
        param_1[0x2f] = param_1[0x2f] & 0xfffffffe;
        fStack_20 = *(float *)(param_4 + 0x18) * DAT_00a0f6d0;
        fStack_28 = (float)unaff_retaddr * fStack_20;
        pvStack_24 = (void *)0x0;
        fStack_20 = (float)param_2 * fStack_20;
        if ((*(byte *)(param_1 + 0x2f) & 1) != 0) {
          (**(code **)(*param_1 + 0x5c))();
        }
        FUN_0074b8f0(&fStack_28,&stack0xffffffcc);
        ExceptionList = pvStack_10;
        return;
      }
      if (*(int *)(param_1[5] + 0x10) != 0) {
        FUN_007464e0();
        ExceptionList = pvStack_10;
        return;
      }
    }
    ExceptionList = pvStack_10;
    return;
  }
                    /* WARNING: Subroutine does not return */
  operator_delete(pvStack_24);
}
```

## Open questions

- Confirm calling convention and full signature against callers.
- Recover meaningful types for still-generic parameters.
