...

{
  ...
  if (local_c8 <= DAT_00a0f70c) {
    ...
    *(undefined4 *)(param_1 + 0xcdc) = *(undefined4 *)(param_1 + 0x528);
  }
  else {
  ...
  if ((*(char *)(param_1 + 0x493) == '\0') &&
    ...
    *(undefined4 *)(iVar4 + 0x170) = DAT_00d1a640;
    *(undefined4 *)(iVar4 + 0x174) = DAT_00d1a644;
    *(undefined4 *)(iVar4 + 0x178) = DAT_00d1a648;
    fVar11 = DAT_00d1a64c;
  }
LAB_009175e5:
...
| **Live decompile** | Matches raw body above (identical CF; large SEH frame) |
| **Body** | `009168d0`–`00917603` (**3379** B / `0xD33`) |
| **Entry bytes** | `55 8B EC 83 E4 F0 6A FF 68 12 D3 9A 00 64 A1 ... 81 EC C8 00 00 00` (frame + SEH + `sub esp,0xC8`) |
| **Epilogue bytes** | `... 5F 5E 64 89 0D 00 00 00 00 5B 8B E5 5D C2 14 00` (**`ret 0x14`**) |
| **ABI** | MSVC **stdcall**, **5 stack dwords** (`ret 0x14`); first arg = camera/controller object* |
| **Proposed name** | `Client_Camera_DefaultModeUpdate_Inferred` |
| **Sole caller** | `FUN_0091ce80` @ default arm of mode switch `obj+0xcc4` / `in_EAX[0x331]` |

### Role summary (sealed CF)

1. Build orbit aim point from focus `param_2` XYZ + yaw `obj+0xcc8` + range scales `param_3`/`param_4` and basis `+0x5ac/+0x5b4`.
2. Optional look-yaw accumulate into `obj+0x538` (gated by map/controller flags).
3. Build look orthonormal basis (cross products + `FUN_0076f5f0` normalize).
4. Collision pull-in loop: up to multi-pass; each pass probes **8** directions via `FUN_00915ad0` then `FUN_00916090` (`Client_Map_RaySegmentHitDistance_Inferred`).
5. Write camera world pos to `DAT_00d17944 + 0x228/0x22c/0x230` (snap vs integrate by `DAT_00d1f0a0`).
6. Optional ground-hit Y adjust on focus; optional gamepad look ? write look dir to `*(DAT_00d1b6d8+0x250)+0x170..`.
7. Fallback look vector from basis `+0x5ac..` or globals `DAT_00d1a640..`.

### Key constants (`read_memory`)

| VA | Value | Use |
|---|---|---|
| `0x00aaa68c` | `1.5f` | look-yaw scale candidate |
| `0x009da8a4` | `3.5f` | alternate scale when `DAT_00d1b26d==0` |
| `0x00afe264` | `12.0f` | yaw delta multiplier |
| `0x00a0f70c` | `0.2f` | hit-distance / pull thresholds |
| `0x00aaa9a4` | `~10000f` | initial best-hit budget |
| `0x00a10e78` | `0.05f` | pull epsilon |
| `0x00a0f2a0` | `1.0f` (`g_flOne`) | normalize / blends |
| `0x00aaa668` | `-1.0f` | basis scale fallback |
