# Annotated low-level: FUN_00879240

| Field | Value |
|---|---|
| Stable ID | `aa_00879240` |
| VA | `0x00879240` |
| System | unknown |
| Date | 2026-07-23 |

## Machine-level notes

- Source: raw capture for `aa_00879240`.
- Prefer assembly when decompiler conflicts.
- Recover types for still-generic parameters via callers/xrefs.
- Map DAT_* globals and FUN_* callees in follow-up waves.

## Pseudocode (annotated copy of raw)

```c
void FUN_00879240(void)

{
  int *piVar1;
  int in_EAX;
  undefined4 uVar2;
  int *piVar3;
  undefined4 *puVar4;
  undefined *puVar5;
  
  if (*(int **)(in_EAX + 0x594) != (int *)0x0) {
    uVar2 = (**(code **)(**(int **)(in_EAX + 0x594) + 0x448))(&DAT_00a328a0);
    *(undefined4 *)(in_EAX + 0x52c) = uVar2;
    puVar5 = &DAT_00a1e460;
    puVar4 = (undefined4 *)(in_EAX + 0x530);
    do {
      uVar2 = (**(code **)(**(int **)(in_EAX + 0x594) + 0x448))(puVar5);
      *puVar4 = uVar2;
      puVar5 = puVar5 + 0x10;
      puVar4 = puVar4 + 1;
    } while ((int)puVar5 < 0xa1e4d0);
    piVar1 = *(int **)(*(int *)(in_EAX + 0x594) + 0x930);
    if (piVar1 != (int *)0x0) {
      *(undefined1 *)((int)piVar1 + 0x489) = 1;
      piVar3 = (int *)(**(code **)(*piVar1 + 0x170))(0xb);
      piVar1[0x8b] = *piVar3;
      piVar3 = (int *)__RTDynamicCast(piVar1[0xac],0,&CNDUIWindow::RTTI_Type_Descriptor,
                                      &CNDUIDialog::RTTI_Type_Descriptor,0);
      if ((piVar3 != (int *)0x0) && ((int *)piVar3[0x12f] == piVar1)) {
        (**(code **)(*piVar3 + 0x1c8))(0);
        (**(code **)(*piVar3 + 0x3c0))(0);
      }
    }
  }
  if (*(int **)(in_EAX + 0x598) != (int *)0x0) {
    uVar2 = (**(code **)(**(int **)(in_EAX + 0x598) + 0x448))(&DAT_00a328a0);
    *(undefined4 *)(in_EAX + 0x54c) = uVar2;
    uVar2 = (**(code **)(**(int **)(in_EAX + 0x598) + 0x448))("Deathmatch");
    *(undefined4 *)(in_EAX + 0x550) = uVar2;
    uVar2 = (**(code **)(**(int **)(in_EAX + 0x598) + 0x448))("Capture \'N Hold");
    *(undefined4 *)(in_EAX + 0x554) = uVar2;
    piVar1 = *(int **)(*(int *)(in_EAX + 0x598) + 0x930);
    if (piVar1 != (int *)0x0) {
      *(undefined1 *)((int)piVar1 + 0x489) = 1;
      piVar3 = (int *)(**(code **)(*piVar1 + 0x170))(0xb);
      piVar1[0x8b] = *piVar3;
      piVar3 = (int *)__RTDynamicCast(piVar1[0xac],0,&CNDUIWindow::RTTI_Type_Descriptor,
                                      &CNDUIDialog::RTTI_Type_Descriptor,0);
      if ((piVar3 != (int *)0x0) && ((int *)piVar3[0x12f] == piVar1)) {
        (**(code **)(*piVar3 + 0x1c8))(0);
        (**(code **)(*piVar3 + 0x3c0))(0);
      }
    }
  }
  return;
}
```

## Open questions

- Confirm calling convention and full signature against callers.
- Recover meaningful types for still-generic parameters.
