# Annotated low-level: Client_NpcDialog_PrepareResponseOpcode
...
- Source: raw capture for `aa_008abd70`.
- Prefer assembly when decompiler conflicts.
- Recover types for still-generic parameters via callers/xrefs.
- Map DAT_* globals and FUN_* callees in follow-up waves.
- **Flush relationship (2026-07-29):** stamps `dialog+0x650 = 0x206E` only. Does not send.
  Downstream send is `Client_MissionDialog_FlushPreparedResponse` @ `0x008ab8f0` (size `0x20`).
  HandleButton fills `+0x654..`. See `reviews/CHAIN_2026-07-29_c2s_206e_seal.md`.
