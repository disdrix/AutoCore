# Annotated low-level: FUN_0044bbc0

| Field | Value |
|---|---|
| Stable ID | `aa_0044bbc0` |
| VA | `0x0044bbc0` |
| System | unknown |
| Date | 2026-07-23 |

## Machine-level notes

- Source: raw capture for `aa_0044bbc0`.
- Prefer assembly when decompiler conflicts.
- Recover types for still-generic parameters via callers/xrefs.
- Map DAT_* globals and FUN_* callees in follow-up waves.

## Pseudocode (annotated copy of raw)

```c
void FUN_0044bbc0(int param_1,undefined4 param_2)

{
  int *piVar1;
  int *piVar2;
  int iVar3;
  int *piVar4;
  int unaff_EDI;
  void *pvStack_c;
  undefined1 *puStack_8;
  undefined4 local_4;
  
  local_4 = 0xffffffff;
  puStack_8 = &LAB_009bf04b;
  pvStack_c = ExceptionList;
  ExceptionList = &pvStack_c;
  if (*(int *)(unaff_EDI + 4) == 0) {
    ExceptionList = &pvStack_c;
    iVar3 = FUN_00414cd0();
    local_4 = 0;
    if (iVar3 == 0) {
      piVar4 = (int *)0x0;
    }
    else {
      piVar4 = (int *)FUN_00999960();
    }
    local_4 = 0xffffffff;
    if ((piVar4 != (int *)0x0) && (piVar4[1] = piVar4[1] + 1, piVar4[1] == 1)) {
      (**(code **)(*piVar4 + 4))();
    }
    piVar2 = *(int **)(unaff_EDI + 4);
    if (piVar2 != (int *)0x0) {
      piVar1 = piVar2 + 1;
      *piVar1 = *piVar1 + -1;
      if (*piVar1 == 0) {
        (**(code **)(*piVar2 + 8))();
      }
    }
    *(int **)(unaff_EDI + 4) = piVar4;
  }
  FUN_0099a3b0(*(undefined4 *)(unaff_EDI + 4),*(undefined4 *)(param_1 + 4),param_2);
  ExceptionList = pvStack_c;
  return;
}
```

## Open questions

- Confirm calling convention and full signature against callers.
- Recover meaningful types for still-generic parameters.
