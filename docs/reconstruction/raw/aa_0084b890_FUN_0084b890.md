# Raw capture: FUN_0084b890

| Field | Value |
|---|---|
| **Stable ID** | `aa_0084b890` |
| **Module** | `autoassault.exe` |
| **Image base** | `0x400000` |
| **VA** | `0x0084b890` |
| **Canonical name** | `FUN_0084b890` |
| **System** | unknown |
| **Capture timestamp** | `2026-07-23` |
| **Tool** | Ghidra MCP `batch_decompile` |
| **Integrity** | Do not overwrite this raw body; append versioned sections only |

---

## Raw pseudocode (authoritative decompile)

```c
/* WARNING: Globals starting with '_' overlap smaller symbols at the same address */

void __fastcall FUN_0084b890(float *param_1)

{
  undefined4 **ppuVar1;
  char cVar2;
  float fVar3;
  uint uVar4;
  undefined4 *puVar5;
  undefined4 uVar6;
  int *piVar7;
  int iVar8;
  int iVar9;
  int *piVar10;
  uint *puVar11;
  int iVar12;
  undefined4 uVar13;
  uint uVar14;
  undefined4 ****ppppuVar15;
  float10 fVar16;
  undefined4 uStack_3f4;
  undefined4 *puStack_3f0;
  undefined4 ***pppuStack_3ec;
  undefined4 *puStack_3e8;
  undefined4 *puStack_3e4;
  undefined4 *puStack_3e0;
  undefined4 ***pppuStack_3dc;
  undefined4 ***pppuStack_3d8;
  undefined4 **ppuStack_3d4;
  undefined4 *puStack_3d0;
  undefined4 uStack_3cc;
  undefined4 ***pppuStack_3c8;
  undefined4 uStack_3c4;
  undefined4 *puStack_3c0;
  undefined4 **ppuStack_3bc;
  undefined4 uStack_3b8;
  undefined4 *puStack_3b4;
  undefined4 uStack_3b0;
  undefined4 uStack_3ac;
  undefined4 **ppuStack_3a8;
  int iStack_3a4;
  undefined4 **ppuStack_3a0;
  undefined4 uStack_39c;
  undefined4 uStack_398;
  undefined4 *puStack_394;
  undefined4 uStack_390;
  undefined4 *puStack_38c;
  undefined4 uStack_388;
  undefined4 **ppuStack_384;
  undefined4 **ppuStack_380;
  undefined4 *puStack_37c;
  undefined4 *puStack_378;
  undefined4 ***pppuStack_374;
  undefined4 uStack_370;
  undefined4 *puStack_36c;
  undefined4 *puStack_368;
  undefined4 uStack_364;
  undefined4 uStack_360;
  undefined4 *puStack_35c;
  undefined4 uStack_358;
  undefined4 ***pppuStack_354;
  undefined4 uStack_350;
  undefined4 uStack_34c;
  undefined4 *puStack_348;
  undefined4 *puStack_344;
  undefined4 uStack_340;
  undefined4 **ppuStack_33c;
  undefined4 *puStack_338;
  undefined4 uStack_334;
  undefined4 *puStack_330;
  undefined4 uStack_32c;
  undefined4 *puStack_328;
  undefined4 uStack_324;
  undefined4 uStack_320;
  undefined4 uStack_31c;
  undefined4 **ppuStack_318;
  undefined4 uStack_314;
  undefined4 *puStack_310;
  undefined4 uStack_30c;
  undefined4 uStack_308;
  undefined4 ***pppuStack_304;
  undefined4 *puStack_300;
  undefined4 uStack_2fc;
  undefined4 uStack_2f8;
  undefined4 *puStack_2f4;
  undefined4 ***pppuStack_2f0;
  undefined4 uStack_2ec;
  undefined4 *puStack_2e8;
  undefined4 *puStack_2e4;
  undefined4 uStack_2e0;
  undefined4 uStack_2dc;
  undefined4 *puStack_2d8;
  undefined4 uStack_2d4;
  undefined4 *puStack_2d0;
  undefined4 uStack_2cc;
  undefined4 uStack_2c8;
  undefined4 **ppuStack_2c4;
  undefined4 *puStack_2c0;
  undefined4 uStack_2bc;
  undefined4 **ppuStack_2b8;
  undefined4 uStack_2b4;
  undefined4 *puStack_2b0;
  undefined4 *puStack_2ac;
  undefined4 uStack_2a8;
  undefined4 uStack_2a4;
  undefined4 uStack_2a0;
  undefined4 uStack_29c;
  uint **ppuStack_298;
  undefined4 uStack_294;
  undefined4 uStack_290;
  undefined4 *puStack_28c;
  undefined4 *puStack_288;
  undefined4 uStack_284;
  undefined4 *puStack_280;
  undefined4 *puStack_27c;
  undefined4 uStack_278;
  undefined4 uStack_274;
  undefined4 **ppuStack_270;
  undefined4 uStack_26c;
  undefined4 *puStack_268;
  undefined4 uStack_264;
  undefined4 uStack_260;
  uint *puStack_25c;
  undefined4 *puStack_258;
  undefined4 uStack_254;
  undefined4 uStack_250;
  undefined4 **ppuStack_24c;
  uint uStack_248;
  undefined4 uStack_244;
  undefined4 *puStack_240;
  int *piStack_23c;
  undefined4 uStack_238;
  undefined4 uStack_234;
  undefined4 *puStack_230;
  undefined4 uStack_22c;
  undefined4 *puStack_228;
  undefined4 uStack_224;
  float fStack_220;
  undefined4 uStack_21c;
  undefined4 uStack_218;
  int *piStack_214;
  undefined4 *puStack_210;
  undefined4 uStack_20c;
  undefined4 *puStack_208;
  undefined4 uStack_204;
  int iStack_200;
  undefined4 **ppuStack_1fc;
  undefined4 *puStack_1f8;
  undefined4 uStack_1f4;
  undefined4 uStack_1f0;
  undefined4 uStack_1ec;
  undefined4 *puStack_1e8;
  undefined4 *puStack_1e4;
  undefined4 uStack_1e0;
  undefined4 uStack_1dc;
  undefined4 *puStack_1d8;
  undefined4 uStack_1d4;
  undefined4 uStack_1d0;
  int *piStack_1cc;
  undefined4 *puStack_1c8;
  undefined4 uStack_1c4;
  undefined1 **ppuStack_1c0;
  undefined4 uStack_1bc;
  int iStack_1b8;
  undefined1 ***pppuStack_1b4;
  int ***pppiStack_1b0;
  undefined4 uStack_1ac;
  undefined4 uStack_1a8;
  undefined4 uStack_1a4;
  undefined1 **ppuStack_1a0;
  char **ppcStack_19c;
  int *piStack_198;
  undefined1 *puStack_194;
  undefined4 uStack_190;
  undefined1 *puStack_18c;
  undefined4 uStack_188;
  int iStack_184;
  char **ppcStack_180;
  float **ppfStack_17c;
  double dStack_170;
  char *pcStack_16c;
  char *pcStack_168;
  char *pcStack_164;
  float fStack_160;
  float *pfStack_15c;
  float local_148;
  int iStack_144;
  undefined4 local_13c;
  undefined1 uStack_132;
  undefined1 uStack_131;
  undefined4 uStack_124;
  undefined4 uStack_120;
  char acStack_114 [272];
  
  if (param_1[0x146] != 0.0) {
    pfStack_15c = (float *)0x84b8b4;
    fVar3 = (float)FUN_007a69d0();
    fStack_160 = 1.2188578e-38;
    pfStack_15c = param_1;
    local_148 = fVar3;
    FUN_0084b210();
    *(undefined4 *)((int)param_1[0x14d] + 0x488) = 1;
    if (*(short *)(*(int *)(*(int *)((int)param_1[0x146] + 0xa8) + 0x3c) + 0x3f4) == 8) {
      pfStack_15c = *(float **)((int)param_1[0x146] + 200);
      fStack_160 = 1.2188652e-38;
      uVar4 = FUN_00519d20();
      pfStack_15c = (float *)(uVar4 | 0xdf000000);
      fStack_160 = 1.2188685e-38;
      (**(code **)(*(int *)param_1[0x14d] + 0x1a0))();
      fStack_160 = 1.2188694e-38;
      FUN_0075bf40();
    }
    else {
      local_13c = 0;
      pfStack_15c = (float *)0x0;
      fStack_160 = 1.2188752e-38;
      (**(code **)(*(int *)param_1[0x14d] + 0x1a0))();
      fStack_160 = 1.2188762e-38;
      FUN_0075bf40();
    }
    if (((*(uint *)((int)param_1[0x146] + 0x17c) >> 0x13 & 1) == 0) && (DAT_00d1791c == '\0')) {
      pfStack_15c = (float *)0x84b976;
      (**(code **)(*(int *)param_1[0x14d] + 0x58))();
    }
    else {
      piVar7 = (int *)param_1[0x14d];
      iVar8 = *piVar7;
      pfStack_15c = (float *)0x84b98c;
      pfStack_15c = (float *)(**(code **)(*piVar7 + 0x2d8))();
      fStack_160 = 1.2188863e-38;
      fStack_160 = (float)(**(code **)(*piVar7 + 0x2c8))();
      pcStack_164 = "i_g_2d_overlay_broken.dds";
      pcStack_168 = (char *)0x84b9a6;
      (**(code **)(iVar8 + 0x50))();
      fVar3 = local_148;
    }
    pfStack_15c = (float *)0x84b9b5;
    uStack_132 = FUN_00845b40();
    pfStack_15c = (float *)0x84b9c4;
    uStack_131 = FUN_00845af0();
    local_148 = (float)(int)((float)DAT_00d1e818 * *(float *)((int)param_1[0x14b] + 0x254) *
                            DAT_00aaa67c);
    iStack_144 = (int)((float)DAT_00d1e81c * *(float *)((int)param_1[0x14b] + 600) * DAT_00aaa678);
    pfStack_15c = &local_148;
    fStack_160 = 1.2189062e-38;
    (**(code **)(*(int *)param_1[0x14b] + 300))();
    fStack_160 = 1.4013e-45;
    pcStack_164 = &stack0xfffffeb4;
    pcStack_168 = (char *)0x84ba3a;
    puVar5 = (undefined4 *)(**(code **)(*(int *)param_1[0x14f] + 0x140))();
    uStack_124 = *puVar5;
    uStack_120 = puVar5[1];
    pcStack_168 = (char *)0x84ba55;
    (**(code **)(*(int *)param_1[0x14f] + 0x268))();
    if (DAT_00d1791c != '\0') {
      if (DAT_00d1b6d8 == 0) {
        local_148 = 0.0;
      }
      else {
        pcStack_168 = (char *)param_1[0x146];
        dStack_170 = (double)CONCAT44(0x84ba7d,uVar6);
        fVar16 = (float10)FUN_0052e640();
        local_148 = (float)fVar16;
      }
      local_148 = local_148 * DAT_00aaa7ac;
      if (DAT_00aaa7ac < local_148) {
        local_148 = DAT_00aaa7ac;
      }
      pcStack_168 = (char *)0xffffffff;
      dStack_170 = 1.611601007019218e-305;
      pcStack_168 = (char *)FUN_007a6de0();
      dStack_170 = (double)local_148;
      ppfStack_17c = (float **)0x84bace;
      sprintf(acStack_114,"%.01f%% %s");
      pcStack_168 = (char *)0xffffff33;
      dStack_170 = (double)CONCAT44(acStack_114,0x84bae9);
      (**(code **)(*(int *)param_1[0x14f] + 0x224))();
    }
    if ((((uint)((int *)param_1[0x146])[0x5f] >> 0x13 & 1) == 0) && (DAT_00d1791c == '\0')) {
      iVar8 = *(int *)param_1[0x14f];
      pcStack_168 = (char *)0x84bb15;
      (**(code **)(*(int *)param_1[0x146] + 0x15c))();
      dStack_170 = (double)CONCAT44(0x84bb1d,uVar6);
      pcStack_168 = (char *)fVar3;
      pcStack_168 = (char *)FUN_007a6a30();
      dStack_170 = (double)CONCAT44(0x84bb2a,uVar6);
      (**(code **)(iVar8 + 0x250))();
    }
    else {
      pcStack_168 = (char *)0xffffffff;
      dStack_170 = (double)CONCAT44(0x84bb3c,uVar6);
      uVar6 = (**(code **)(*(int *)param_1[0x146] + 0x15c))();
      dStack_170 = (double)CONCAT44(uVar6,0x84bb44);
      pcStack_168 = (char *)FUN_007a6de0();
      dStack_170 = -NAN;
      uVar6 = FUN_007a6de0();
      dStack_170 = (double)CONCAT44(uVar6,"%s: %s");
      sprintf(acStack_114,"%s: %s");
      pcStack_168 = acStack_114;
      dStack_170 = (double)CONCAT44(0x84bb76,uVar6);
      (**(code **)(*(int *)param_1[0x14f] + 0x250))();
    }
    iVar8 = *(int *)param_1[0x14f];
    dStack_170 = (double)CONCAT44(&stack0xfffffea8,0x84bb8e);
    uVar6 = FUN_00514050();
    dStack_170 = (double)CONCAT44(uVar6,1);
    (**(code **)(iVar8 + 0x15c))();
    fStack_160 = (float)DAT_00d1e81c * *(float *)((int)param_1[0x14f] + 600) * DAT_00aaa678;
    ppfStack_17c = (float **)0x84bbd6;
    piVar7 = (int *)(**(code **)(*(int *)param_1[0x14b] + 0x140))();
    pfStack_15c = (float *)(*piVar7 + -10);
    ppfStack_17c = &pfStack_15c;
    ppcStack_180 = (char **)0x84bbfc;
    (**(code **)(*(int *)param_1[0x14f] + 300))();
    ppcStack_180 = &pcStack_16c;
    dStack_170 = (double)CONCAT44(5,uVar6);
    pcStack_168 = (char *)0x6;
    iStack_184 = 0x84bc20;
    (**(code **)(*(int *)param_1[0x14f] + 0x110))();
    iStack_184 = 0x84bc2e;
    (**(code **)(*(int *)param_1[0x14f] + 0x13c))();
    piVar7 = (int *)param_1[0x14f];
    iStack_184 = 0;
    uStack_188 = 1;
    puStack_18c = &stack0xfffffea8;
    uStack_190 = 0x84bc46;
    ppfStack_17c = (float **)(**(code **)(*piVar7 + 0x120))();
    uStack_190 = 1;
    puStack_194 = (undefined1 *)&stack0xfffffe90;
    piStack_198 = (int *)0x84bc5b;
    iVar8 = (**(code **)(*piVar7 + 0x140))();
    iVar8 = *(int *)(iStack_184 + 4) + 1 + *(int *)(iVar8 + 4);
    piStack_198 = (int *)0x84bc77;
    (**(code **)(*(int *)param_1[0x151] + 0x268))();
    piStack_198 = (int *)param_1[0x146];
    ppcStack_19c = (char **)param_1[0x151];
    ppuStack_1a0 = (undefined1 **)0x84bc8a;
    iVar9 = FUN_00845880();
    if (iVar9 < 1) {
      piStack_198 = &iStack_184;
      iStack_184 = 0;
      ppcStack_180 = (char **)0x0;
      ppcStack_19c = (char **)0x84bd6d;
      (**(code **)(*(int *)param_1[0x151] + 300))();
      ppcStack_19c = (char **)0x0;
      ppuStack_1a0 = (undefined1 **)0x84bd7c;
      (**(code **)(*(int *)param_1[0x151] + 0xcc))();
    }
    else {
      piStack_198 = (int *)0x1;
      ppcStack_19c = &pcStack_16c;
      ppuStack_1a0 = (undefined1 **)0x84bcaa;
      piVar7 = (int *)(**(code **)(*(int *)param_1[0x14f] + 0x140))();
      puStack_18c = (undefined1 *)*piVar7;
      ppuStack_1a0 = &puStack_18c;
      uStack_188 = 0;
      uStack_1a4 = 0x84bcc4;
      (**(code **)(*(int *)param_1[0x151] + 8))();
      uStack_1a4 = 0x84bcd2;
      (**(code **)(*(int *)param_1[0x151] + 0x13c))();
      uStack_1a4 = 1;
      uStack_1a8 = 0x84bce2;
      (**(code **)(*(int *)param_1[0x151] + 0xcc))();
      uStack_1a8 = 0;
      uStack_1ac = 1;
      pppiStack_1b0 = (int ***)&ppfStack_17c;
      pppuStack_1b4 = (undefined1 ***)0x84bcf8;
      puVar5 = (undefined4 *)(**(code **)(*(int *)param_1[0x14f] + 0x120))();
      ppuStack_1a0 = (undefined1 **)*puVar5;
      pppuStack_1b4 = &ppuStack_1a0;
      iStack_1b8 = 0x84bd15;
      ppcStack_19c = (char **)iVar8;
      (**(code **)(*(int *)param_1[0x151] + 0x118))();
      piVar7 = (int *)param_1[0x151];
      iStack_1b8 = 0;
      uStack_1bc = 1;
      ppuStack_1c0 = &puStack_18c;
      uStack_1c4 = 0x84bd2d;
      pppiStack_1b0 = (int ***)(**(code **)(*piVar7 + 0x120))();
      uStack_1c4 = 1;
      puStack_1c8 = &uStack_1a4;
      piStack_1cc = (int *)0x84bd42;
      iVar8 = (**(code **)(*piVar7 + 0x140))();
      iVar8 = *(int *)(iStack_1b8 + 4) + 1 + *(int *)(iVar8 + 4);
    }
    piStack_1cc = &iStack_1b8;
    iStack_1b8 = 5;
    uStack_1d0 = 0x84bd97;
    pppuStack_1b4 = (undefined1 ***)iVar8;
    (**(code **)(*(int *)param_1[0x14d] + 0x110))();
    uStack_1d0 = 0;
    uStack_1d4 = 1;
    puStack_1d8 = &uStack_1a4;
    uStack_1dc = 0x84bdad;
    iVar9 = (**(code **)(*(int *)param_1[0x14d] + 0x120))();
    uVar13 = *(undefined4 *)(iVar9 + 4);
    uStack_1dc = 0;
    uStack_1e0 = 1;
    puStack_1e4 = &uStack_1bc;
    puStack_1e8 = (undefined4 *)0x84bdc6;
    puVar5 = (undefined4 *)(**(code **)(*(int *)param_1[0x14d] + 0x120))();
    uStack_1d4 = *puVar5;
    puStack_1e8 = &uStack_1d4;
    uStack_1ec = 0x84bde3;
    uStack_1d0 = uVar13;
    (**(code **)(*(int *)param_1[0x14e] + 0x110))();
    uStack_1ec = 0x84bdf1;
    (**(code **)(*(int *)param_1[0x150] + 0x268))();
    uStack_1f0 = (undefined1 ***)param_1[0x146];
    uStack_1ec = 1;
    uStack_1f4 = 0x84be05;
    FUN_00847240();
    uStack_1ec = 1;
    uStack_1f0 = &ppuStack_1c0;
    uStack_1f4 = 0x84be1d;
    piVar7 = (int *)(**(code **)(*(int *)param_1[0x14e] + 0x140))();
    uStack_1f4 = 1;
    puStack_1f8 = &uStack_1d4;
    ppuStack_1fc = (undefined4 **)0x84be34;
    piVar10 = (int *)(**(code **)(*(int *)param_1[0x14f] + 0x140))();
    ppuStack_1fc = &puStack_1e8;
    puStack_1e8 = (undefined4 *)((*piVar10 - *piVar7) + -1);
    puStack_1e4 = (undefined4 *)0x0;
    iStack_200 = 0x84be56;
    (**(code **)(*(int *)param_1[0x150] + 300))();
    iStack_200 = 0x84be64;
    (**(code **)(*(int *)param_1[0x150] + 0x13c))();
    iStack_200 = 0;
    uStack_204 = 1;
    puStack_208 = &uStack_1d4;
    uStack_20c = 0x84be7a;
    piVar7 = (int *)(**(code **)(*(int *)param_1[0x14e] + 0x120))();
    uStack_20c = 1;
    puStack_210 = &uStack_1ec;
    piStack_214 = (int *)0x84be91;
    piVar10 = (int *)(**(code **)(*(int *)param_1[0x14e] + 0x140))();
    iStack_200 = *piVar7 + 1 + *piVar10;
    piStack_214 = &iStack_200;
    uStack_218 = 0x84beb4;
    ppuStack_1fc = (undefined4 **)iVar8;
    (**(code **)(*(int *)param_1[0x150] + 0x118))();
    if (uStack_1f0._3_1_ == '\0') {
      uStack_218 = 0;
      uStack_21c = 0x84c040;
      (**(code **)(*(int *)param_1[0x157] + 0xcc))();
      uStack_21c = 0;
      fStack_220 = 1.2191273e-38;
      (**(code **)(*(int *)param_1[0x158] + 0xcc))();
    }
    else {
      uStack_218 = 1;
      uStack_21c = 0x84bece;
      (**(code **)(*(int *)param_1[0x157] + 0xcc))();
      uStack_21c = 1;
      fStack_220 = 1.2190756e-38;
      (**(code **)(*(int *)param_1[0x158] + 0xcc))();
      fStack_220 = 1.2190775e-38;
      iVar8 = (**(code **)(*(int *)param_1[0x146] + 0x1e0))();
      if (iVar8 != 0) {
        fStack_220 = 1.21908e-38;
        iVar8 = (**(code **)(*(int *)param_1[0x146] + 0x1e0))();
        fStack_220 = *(float *)(iVar8 + 0xdc);
        iVar8 = *(int *)param_1[0x157];
        uStack_224 = 0x84bf12;
        fVar16 = (float10)FUN_0040d0a0();
        fStack_220 = (float)(fVar16 * (float10)_DAT_00aaa99c);
        uStack_224 = 0x84bf23;
        (**(code **)(iVar8 + 0x3ac))();
        uStack_224 = 0x84bf31;
        iVar8 = (**(code **)(*(int *)param_1[0x146] + 0x1e0))();
        uStack_224 = *(undefined4 *)(iVar8 + 0xe8);
        puStack_228 = (undefined4 *)0x84bf46;
        (**(code **)(*(int *)param_1[0x157] + 0x3b0))();
      }
      if (DAT_00d1b6d8 != 0) {
        iVar8 = *(int *)param_1[0x157];
        fStack_220 = 1.2190973e-38;
        fStack_220 = (float)FUN_00930ea0();
        uStack_224 = 0;
        puStack_228 = (undefined4 *)0x84bf83;
        (**(code **)(iVar8 + 0x15c))();
      }
      piVar7 = (int *)param_1[0x14e];
      fStack_220 = 0.0;
      uStack_224 = 1;
      puStack_228 = &uStack_1f4;
      uStack_22c = 0x84bf9b;
      iVar9 = (**(code **)(*piVar7 + 0x120))();
      uStack_22c = 1;
      puStack_230 = &uStack_20c;
      uStack_234 = 0x84bfae;
      iVar8 = (**(code **)(*piVar7 + 0x140))();
      iVar8 = *(int *)(iVar8 + 4);
      iVar9 = *(int *)(iVar9 + 4);
      uStack_234 = 0;
      uStack_238 = 1;
      piStack_23c = &iStack_200;
      puStack_240 = (undefined4 *)0x84bfce;
      puVar5 = (undefined4 *)(**(code **)(*(int *)param_1[0x14e] + 0x120))();
      uStack_22c = *puVar5;
      puStack_240 = &uStack_22c;
      uStack_244 = 0x84bfeb;
      puStack_228 = (undefined4 *)(iVar9 + 1 + iVar8);
      (**(code **)(*(int *)param_1[0x157] + 0x110))();
      uStack_244 = 0;
      uStack_248 = 1;
      ppuStack_24c = &puStack_210;
      uStack_250 = 0x84c001;
      iVar8 = (**(code **)(*(int *)param_1[0x157] + 0x120))();
      uVar13 = *(undefined4 *)(iVar8 + 4);
      uStack_250 = 0;
      uStack_254 = 1;
      puStack_258 = &uStack_224;
      puStack_25c = (uint *)0x84c01a;
      puVar11 = (uint *)(**(code **)(*(int *)param_1[0x157] + 0x120))();
      uStack_248 = *puVar11;
      puStack_25c = &uStack_248;
      uStack_260 = 0x84c037;
      uStack_244 = uVar13;
      (**(code **)(*(int *)param_1[0x158] + 0x110))();
    }
    uStack_260 = 0x84c05d;
    (**(code **)(*(int *)param_1[0x152] + 0x268))();
    uStack_260 = 0x84c06b;
    (**(code **)(*(int *)param_1[0x155] + 0x268))();
    if (DAT_00d1b6d8 != 0) {
      uVar4 = *(uint *)((int)param_1[0x146] + 0x160);
      uStack_248 = *(uint *)((int)param_1[0x146] + 0x164);
      if ((uVar4 & uStack_248) != 0xffffffff) {
        uVar14 = 0;
        do {
          if ((*(uint *)(DAT_00d1b6d8 + 0x5a0 + (uVar14 & 0xff) * 8) == uVar4) &&
             (*(uint *)(DAT_00d1b6d8 + 0x5a4 + (uVar14 & 0xff) * 8) == uStack_248)) {
            DAT_00d1791c = '\x01';
            break;
          }
          uVar14 = uVar14 + 1;
        } while ((int)uVar14 < 8);
      }
    }
    puStack_268 = (undefined4 *)param_1[0x146];
    uStack_260 = 0;
    uStack_264 = 1;
    uStack_26c = 0x84c0e1;
    UI_BuildItemTooltipStats();
    piVar7 = (int *)param_1[0x151];
    uStack_260 = 0;
    uStack_264 = 1;
    puStack_268 = &uStack_22c;
    uStack_26c = 0x84c0fc;
    iVar9 = (**(code **)(*piVar7 + 0x120))();
    uStack_26c = 1;
    ppuStack_270 = &puStack_240;
    uStack_274 = 0x84c10f;
    iVar8 = (**(code **)(*piVar7 + 0x140))();
    iVar8 = *(int *)(iVar8 + 4);
    iVar9 = *(int *)(iVar9 + 4);
    uStack_274 = 0;
    uStack_278 = 1;
    puStack_27c = &uStack_254;
    puStack_280 = (undefined4 *)0x84c12f;
    puVar5 = (undefined4 *)(**(code **)(*(int *)param_1[0x152] + 0x120))();
    uStack_26c = *puVar5;
    puStack_280 = &uStack_26c;
    uStack_284 = 0x84c14c;
    puStack_268 = (undefined4 *)(iVar9 + 2 + iVar8);
    (**(code **)(*(int *)param_1[0x152] + 0x118))();
    uStack_284 = 1;
    puStack_288 = &uStack_250;
    puStack_28c = (undefined4 *)0x84c161;
    puVar5 = (undefined4 *)(**(code **)(*(int *)param_1[0x150] + 0x140))();
    uStack_278 = *puVar5;
    puStack_28c = &uStack_278;
    uStack_274 = 0;
    uStack_290 = 0x84c17e;
    (**(code **)(*(int *)param_1[0x152] + 300))();
    piVar7 = (int *)param_1[0x150];
    uStack_290 = 0;
    uStack_294 = 1;
    ppuStack_298 = &puStack_25c;
    uStack_29c = 0x84c196;
    iVar9 = (**(code **)(*piVar7 + 0x120))();
    uStack_29c = 1;
    uStack_2a0 = &ppuStack_270;
    uStack_2a4 = 0x84c1a9;
    iVar8 = (**(code **)(*piVar7 + 0x140))();
    iVar8 = *(int *)(iVar8 + 4);
    iVar9 = *(int *)(iVar9 + 4);
    uStack_2a4 = 0;
    uStack_2a8 = 1;
    puStack_2ac = &uStack_284;
    puStack_2b0 = (undefined4 *)0x84c1c9;
    puVar5 = (undefined4 *)(**(code **)(*(int *)param_1[0x150] + 0x120))();
    uStack_29c = *puVar5;
    puStack_2b0 = &uStack_29c;
    uStack_2b4 = 0x84c1e6;
    ppuStack_298 = (uint **)(iVar9 + 1 + iVar8);
    (**(code **)(*(int *)param_1[0x152] + 0x110))();
    uStack_2b4 = 0x84c1f4;
    (**(code **)(*(int *)param_1[0x152] + 0x13c))();
    uStack_2b4 = 1;
    ppuStack_2b8 = &puStack_280;
    uStack_2bc = 0x84c209;
    iVar8 = (**(code **)(*(int *)param_1[0x152] + 0x140))();
    uVar13 = *(undefined4 *)(iVar8 + 4);
    uStack_2bc = 1;
    puStack_2c0 = &uStack_290;
    ppuStack_2c4 = (undefined4 **)0x84c221;
    puStack_2b0 = (undefined4 *)(**(code **)(*(int *)param_1[0x150] + 0x140))();
    puStack_2b0 = (undefined4 *)*puStack_2b0;
    ppuStack_2c4 = &puStack_2b0;
    uStack_2c8 = 0x84c23e;
    puStack_2ac = (undefined4 *)uVar13;
    (**(code **)(*(int *)param_1[0x152] + 300))();
    if (uStack_2a0._2_1_ == '\0') {
      uStack_2c8 = 0;
      uStack_2cc = 0x84c31c;
      (**(code **)(*(int *)param_1[0x155] + 4))();
    }
    else {
      uStack_2c8 = 0x84c259;
      FUN_00845be0();
      piVar7 = (int *)param_1[0x152];
      uStack_2c8 = 0;
      uStack_2cc = 1;
      puStack_2d0 = &uStack_294;
      uStack_2d4 = 0x84c271;
      iVar9 = (**(code **)(*piVar7 + 0x120))();
      uStack_2d4 = 1;
      puStack_2d8 = &uStack_2a8;
      uStack_2dc = 0x84c284;
      iVar8 = (**(code **)(*piVar7 + 0x140))();
      iVar8 = *(int *)(iVar8 + 4);
      iVar9 = *(int *)(iVar9 + 4);
      uStack_2dc = 0;
      uStack_2e0 = 1;
      puStack_2e4 = &uStack_2bc;
      puStack_2e8 = (undefined4 *)0x84c2a4;
      puVar5 = (undefined4 *)(**(code **)(*(int *)param_1[0x152] + 0x120))();
      uStack_2d4 = *puVar5;
      puStack_2e8 = &uStack_2d4;
      uStack_2ec = 0x84c2c1;
      puStack_2d0 = (undefined4 *)(iVar9 + 1 + iVar8);
      (**(code **)(*(int *)param_1[0x155] + 0x110))();
      uStack_2ec = 1;
      pppuStack_2f0 = &ppuStack_2b8;
      puStack_2f4 = (undefined4 *)0x84c2d6;
      puVar5 = (undefined4 *)(**(code **)(*(int *)param_1[0x152] + 0x140))();
      uStack_2e0 = *puVar5;
      puStack_2f4 = &uStack_2e0;
      uStack_2dc = 0;
      uStack_2f8 = 0x84c2f3;
      (**(code **)(*(int *)param_1[0x155] + 300))();
      uStack_2f8 = 0x84c301;
      (**(code **)(*(int *)param_1[0x155] + 0x13c))();
      uStack_2f8 = 1;
      uStack_2fc = 0x84c30e;
      (**(code **)(*(int *)param_1[0x155] + 4))();
    }
    uStack_2fc = 0x84c32a;
    (**(code **)(*(int *)param_1[0x153] + 0x268))();
    uStack_2fc = 1;
    puStack_300 = &uStack_2c8;
    pppuStack_304 = (undefined4 ***)0x84c33f;
    puVar5 = (undefined4 *)(**(code **)(*(int *)param_1[0x14e] + 0x140))();
    pppuStack_2f0 = (undefined4 ***)*puVar5;
    pppuStack_304 = &pppuStack_2f0;
    uStack_2ec = 0;
    uStack_308 = 0x84c35c;
    (**(code **)(*(int *)param_1[0x153] + 300))();
    uStack_308 = 0;
    puStack_310 = &uStack_2d4;
    uStack_30c = 1;
    if (uStack_2e0._3_1_ == '\0') {
      piVar7 = (int *)param_1[0x14e];
      uStack_314 = 0x84c3af;
      iVar8 = (**(code **)(*piVar7 + 0x120))();
      uStack_314 = 1;
      ppuStack_318 = &puStack_2e8;
      uStack_31c = 0x84c3c2;
      iVar9 = (**(code **)(*piVar7 + 0x140))();
      iVar8 = *(int *)(iVar8 + 4) + 1 + *(int *)(iVar9 + 4);
      piVar7 = (int *)param_1[0x14e];
    }
    else {
      piVar7 = (int *)param_1[0x158];
      uStack_314 = 0x84c37a;
      iVar8 = (**(code **)(*piVar7 + 0x120))();
      uStack_314 = 1;
      ppuStack_318 = &puStack_2e8;
      uStack_31c = 0x84c38d;
      iVar9 = (**(code **)(*piVar7 + 0x140))();
      iVar8 = *(int *)(iVar8 + 4) + 1 + *(int *)(iVar9 + 4);
      piVar7 = (int *)param_1[0x158];
    }
    uStack_31c = 0;
    uStack_324 = &uStack_2fc;
    uStack_320 = 1;
    puStack_328 = (undefined4 *)0x84c3e2;
    puVar5 = (undefined4 *)(**(code **)(*piVar7 + 0x120))();
    uStack_314 = *puVar5;
    puStack_328 = &uStack_314;
    uStack_32c = 0x84c3ff;
    puStack_310 = (undefined4 *)iVar8;
    (**(code **)(*(int *)param_1[0x153] + 0x110))();
    puStack_330 = (undefined4 *)param_1[0x153];
    uStack_32c = 1;
    uStack_334 = 0x84c413;
    FUN_00845360();
    uStack_32c = 0x84c424;
    (**(code **)(*(int *)param_1[0x153] + 0x13c))();
    uStack_32c = 1;
    puStack_330 = &uStack_2f8;
    uStack_334 = 0x84c439;
    iVar8 = (**(code **)(*(int *)param_1[0x153] + 0x140))();
    uVar13 = *(undefined4 *)(iVar8 + 4);
    uStack_334 = 1;
    puStack_338 = &uStack_308;
    ppuStack_33c = (undefined4 **)0x84c451;
    puStack_328 = (undefined4 *)(**(code **)(*(int *)param_1[0x14e] + 0x140))();
    puStack_328 = (undefined4 *)*puStack_328;
    ppuStack_33c = &puStack_328;
    uStack_340 = 0x84c46e;
    uStack_324 = (undefined4 *)uVar13;
    (**(code **)(*(int *)param_1[0x153] + 300))();
    uStack_340 = 0x84c47c;
    (**(code **)(*(int *)param_1[0x154] + 0x268))();
    uStack_340 = 1;
    puStack_344 = &uStack_30c;
    puStack_348 = (undefined4 *)0x84c491;
    puVar5 = (undefined4 *)(**(code **)(*(int *)param_1[0x152] + 0x140))();
    uStack_334 = *puVar5;
    puStack_348 = &uStack_334;
    puStack_330 = (undefined4 *)0x0;
    uStack_34c = 0x84c4ae;
    (**(code **)(*(int *)param_1[0x154] + 300))();
    uStack_34c = 0;
    pppuStack_354 = &ppuStack_318;
    uStack_350 = 1;
    if (uStack_324._2_1_ == '\0') {
      piVar7 = (int *)param_1[0x152];
      uStack_358 = 0x84c501;
      iVar8 = (**(code **)(*piVar7 + 0x120))();
      uStack_358 = 1;
      puStack_35c = &uStack_32c;
      uStack_360 = 0x84c514;
      iVar9 = (**(code **)(*piVar7 + 0x140))();
      iVar8 = *(int *)(iVar8 + 4) + 1 + *(int *)(iVar9 + 4);
      piVar7 = (int *)param_1[0x152];
    }
    else {
      piVar7 = (int *)param_1[0x155];
      uStack_358 = 0x84c4cc;
      iVar8 = (**(code **)(*piVar7 + 0x120))();
      uStack_358 = 1;
      puStack_35c = &uStack_32c;
      uStack_360 = 0x84c4df;
      iVar9 = (**(code **)(*piVar7 + 0x140))();
      iVar8 = *(int *)(iVar8 + 4) + 1 + *(int *)(iVar9 + 4);
      piVar7 = (int *)param_1[0x155];
    }
    uStack_360 = 0;
    puStack_368 = &uStack_340;
    uStack_364 = 1;
    puStack_36c = (undefined4 *)0x84c534;
    puVar5 = (undefined4 *)(**(code **)(*piVar7 + 0x120))();
    uStack_358 = *puVar5;
    puStack_36c = &uStack_358;
    uStack_370 = 0x84c551;
    pppuStack_354 = (undefined4 ***)iVar8;
    (**(code **)(*(int *)param_1[0x154] + 0x110))();
    pppuStack_374 = (undefined4 ***)param_1[0x154];
    uStack_370 = 1;
    puStack_378 = (undefined4 *)0x84c565;
    FUN_0084b770();
    uStack_370 = 0x84c576;
    (**(code **)(*(int *)param_1[0x154] + 0x13c))();
    uStack_370 = 1;
    pppuStack_374 = &ppuStack_33c;
    puStack_378 = (undefined4 *)0x84c58b;
    iVar8 = (**(code **)(*(int *)param_1[0x154] + 0x140))();
    puVar5 = *(undefined4 **)(iVar8 + 4);
    puStack_378 = (undefined4 *)0x1;
    puStack_37c = &uStack_34c;
    ppuStack_380 = (undefined4 **)0x84c5a3;
    puStack_36c = (undefined4 *)(**(code **)(*(int *)param_1[0x152] + 0x140))();
    puStack_36c = (undefined4 *)*puStack_36c;
    ppuStack_380 = &puStack_36c;
    ppuStack_384 = (undefined4 ***)0x84c5c0;
    puStack_368 = puVar5;
    (**(code **)(*(int *)param_1[0x154] + 300))();
    piVar7 = (int *)param_1[0x154];
    piVar10 = (int *)param_1[0x153];
    ppuStack_384 = (undefined4 ***)0x0;
    uStack_388 = 1;
    puStack_38c = &uStack_350;
    uStack_390 = 0x84c5e1;
    puStack_37c = (undefined4 *)(**(code **)(*piVar7 + 0x120))();
    uStack_390 = 1;
    puStack_394 = &uStack_364;
    uStack_398 = 0x84c5f6;
    iVar8 = (**(code **)(*piVar7 + 0x140))();
    ppuVar1 = (undefined4 **)ppuStack_384[1];
    iVar8 = *(int *)(iVar8 + 4);
    uStack_398 = 0;
    uStack_39c = 1;
    ppuStack_3a0 = &puStack_378;
    iStack_3a4 = 0x84c613;
    iVar9 = (**(code **)(*piVar10 + 0x120))();
    iStack_3a4 = 1;
    ppuStack_3a8 = &puStack_368;
    uStack_3ac = 0x84c626;
    iVar12 = (**(code **)(*piVar10 + 0x140))();
    puStack_3b4 = &uStack_370;
    uStack_3ac = 0;
    uStack_3b0 = 1;
    if (*(int *)(iVar9 + 4) + *(int *)(iVar12 + 4) < (int)ppuVar1 + iVar8) {
      piVar7 = (int *)param_1[0x154];
      uStack_3b8 = 0x84c649;
      iVar8 = (**(code **)(*piVar7 + 0x120))();
      uStack_3b8 = 1;
      ppuStack_3bc = &ppuStack_384;
      puStack_3c0 = (undefined4 *)0x84c65c;
      iVar9 = (**(code **)(*piVar7 + 0x140))();
      ppppuVar15 = (undefined4 ****)(*(int *)(iVar8 + 4) + *(int *)(iVar9 + 4));
      puStack_3c0 = (undefined4 *)0x0;
      uStack_3c4 = 1;
      pppuStack_3c8 = &ppuStack_384;
      uStack_3cc = 0x84c679;
      iVar8 = (**(code **)(*(int *)param_1[0x153] + 0x120))();
      iVar8 = *(int *)(iVar8 + 4);
      uStack_3cc = 1;
      puStack_3d0 = &uStack_398;
      ppuStack_3d4 = (undefined4 **)0x84c693;
      piVar7 = (int *)(**(code **)(*(int *)param_1[0x153] + 0x140))();
      puStack_3c0 = (undefined4 *)*piVar7;
      ppuStack_3d4 = &puStack_3c0;
      pppuStack_3d8 = (undefined4 ***)0x84c6b0;
      ppuStack_3bc = (undefined4 ***)((int)ppppuVar15 - iVar8);
      (**(code **)(*(int *)param_1[0x153] + 300))();
    }
    else {
      piVar7 = (int *)param_1[0x153];
      uStack_3b8 = 0x84c6c5;
      iVar8 = (**(code **)(*piVar7 + 0x120))();
      uStack_3b8 = 1;
      ppuStack_3bc = &ppuStack_384;
      puStack_3c0 = (undefined4 *)0x84c6d8;
      iVar9 = (**(code **)(*piVar7 + 0x140))();
      piVar7 = (int *)param_1[0x154];
      ppppuVar15 = (undefined4 ****)(*(int *)(iVar8 + 4) + *(int *)(iVar9 + 4));
      puStack_3c0 = (undefined4 *)0x0;
      uStack_3c4 = 1;
      if (uStack_398._2_1_ == '\0') {
        piVar10 = (int *)param_1[0x152];
        pppuStack_3c8 = &pppuStack_374;
        uStack_3cc = 0x84c76d;
        uStack_3b8 = (**(code **)(*piVar7 + 0x120))();
        uStack_3cc = 1;
        puStack_3d0 = &uStack_390;
        ppuStack_3d4 = (undefined4 **)0x84c782;
        iVar8 = (**(code **)(*piVar10 + 0x140))();
        iVar8 = *(int *)(iVar8 + 4);
        iVar9 = puStack_3c0[1];
        ppuStack_3d4 = (undefined4 **)0x1;
        pppuStack_3d8 = &ppuStack_3a0;
        pppuStack_3dc = (undefined4 ***)0x84c79d;
        iVar12 = (**(code **)(*piVar7 + 0x140))();
        iVar8 = (iVar8 - iVar9) - *(int *)(iVar12 + 4);
        pppuStack_3dc = (undefined4 ***)0x1;
        puStack_3e0 = &uStack_3b0;
        puStack_3e4 = (undefined4 *)0x84c7b7;
        puVar5 = (undefined4 *)(**(code **)(*(int *)param_1[0x152] + 0x140))();
        uStack_3c4 = *puVar5;
        piVar7 = (int *)param_1[0x152];
      }
      else {
        piVar10 = (int *)param_1[0x155];
        pppuStack_3c8 = &ppuStack_384;
        uStack_3cc = 0x84c704;
        uStack_3b8 = (**(code **)(*piVar7 + 0x120))();
        uStack_3cc = 1;
        puStack_3d0 = &uStack_398;
        ppuStack_3d4 = (undefined4 **)0x84c719;
        iVar8 = (**(code **)(*piVar10 + 0x140))();
        iVar8 = *(int *)(iVar8 + 4);
        iVar9 = puStack_3c0[1];
        ppuStack_3d4 = (undefined4 **)0x1;
        pppuStack_3d8 = &ppuStack_3a8;
        pppuStack_3dc = (undefined4 ***)0x84c734;
        iVar12 = (**(code **)(*piVar7 + 0x140))();
        iVar8 = (iVar8 - iVar9) - *(int *)(iVar12 + 4);
        pppuStack_3dc = (undefined4 ***)0x1;
        puStack_3e0 = &uStack_390;
        puStack_3e4 = (undefined4 *)0x84c74e;
        puVar5 = (undefined4 *)(**(code **)(*(int *)param_1[0x155] + 0x140))();
        uStack_3c4 = *puVar5;
        piVar7 = (int *)param_1[0x155];
      }
      puStack_3c0 = (undefined4 *)(iVar8 + (int)ppppuVar15);
      puStack_3e4 = &uStack_3c4;
      puStack_3e8 = (undefined4 *)0x84c7d4;
      (**(code **)(*piVar7 + 300))();
      puStack_3e8 = (undefined4 *)0x1;
      pppuStack_3ec = (undefined4 ***)&uStack_39c;
      puStack_3f0 = (undefined4 *)0x84c7e9;
      iVar8 = (**(code **)(*(int *)param_1[0x154] + 0x140))();
      puStack_3f0 = (undefined4 *)0x0;
      iVar8 = *(int *)(iVar8 + 4);
      uStack_3f4 = 1;
      puStack_3e8 = (undefined4 *)(**(code **)(*(int *)param_1[0x154] + 0x120))(&puStack_3b4);
      puStack_3e8 = (undefined4 *)*puStack_3e8;
      puStack_3e4 = (undefined4 *)((int)ppppuVar15 - iVar8);
      (**(code **)(*(int *)param_1[0x154] + 0x110))(&puStack_3e8);
    }
    pppuStack_3d8 = (undefined4 ***)0x84c830;
    (**(code **)(*(int *)param_1[0x156] + 0x268))();
    if ((DAT_00d1b6d8 == 0) || (*(int *)(DAT_00d1b6d8 + 0x6b4) < 1)) {
      pppuStack_3d8 = (undefined4 ***)&uStack_3c4;
      uStack_3c4 = 0;
      puStack_3c0 = (undefined4 *)0x0;
      pppuStack_3dc = (undefined4 ***)0x84c8ee;
      (**(code **)(*(int *)param_1[0x156] + 300))();
      pppuStack_3dc = (undefined4 ****)0x0;
      puStack_3e0 = (undefined4 *)0x84c8fd;
      (**(code **)(*(int *)param_1[0x156] + 0xcc))();
    }
    else {
      pppuStack_3d8 = (undefined4 ***)0x1;
      pppuStack_3dc = (undefined4 ***)0x84c85c;
      (**(code **)(*(int *)param_1[0x156] + 0xcc))();
      pppuStack_3c8 =
           (undefined4 ***)
           (int)((float)DAT_00d1e818 * (float)((int *)param_1[0x156])[0x95] * DAT_00aaa67c);
      pppuStack_3dc = &pppuStack_3c8;
      uStack_3c4 = 0;
      puStack_3e0 = (undefined4 *)0x84c893;
      (**(code **)(*(int *)param_1[0x156] + 300))();
      puStack_3e0 = (undefined4 *)0x84c8a4;
      FUN_008452f0();
      puStack_3e0 = (undefined4 *)0x84c8b2;
      (**(code **)(*(int *)param_1[0x156] + 0x13c))();
      puStack_3e0 = &uStack_3cc;
      uStack_3cc = 0;
      puStack_3e4 = (undefined4 *)0x84c8d1;
      pppuStack_3c8 = ppppuVar15;
      (**(code **)(*(int *)param_1[0x156] + 0x110))();
    }
    puStack_3e4 = (undefined4 *)0x1;
    puStack_3e8 = &uStack_398;
    pppuStack_3ec = (undefined4 ***)0x84c912;
    puVar5 = (undefined4 *)(**(code **)(*(int *)param_1[0x14b] + 0x140))();
    ppuStack_3a8 = (undefined4 **)*puVar5;
    uStack_39c = puVar5[1];
    iStack_3a4 = (int)ppppuVar15 + 6;
    pppuStack_3ec = &ppuStack_3a8;
    puStack_3f0 = (undefined4 *)0x84c936;
    (**(code **)(*(int *)param_1[0x14b] + 8))();
    puStack_3f0 = (undefined4 *)0x84c944;
    cVar2 = (**(code **)(*(int *)param_1[0x156] + 0xd0))();
    if (cVar2 == '\0') {
      puStack_3f0 = &uStack_3ac;
      uStack_3f4 = 0x84c9a2;
      (**(code **)((int)*param_1 + 8))();
    }
    else {
      piVar7 = (int *)param_1[0x156];
      puStack_3f0 = (undefined4 *)0x0;
      uStack_3f4 = 1;
      iVar8 = (**(code **)(*piVar7 + 0x120))(&iStack_3a4);
      iVar9 = (**(code **)(*piVar7 + 0x140))(&puStack_3c0,1);
      pppuStack_3ec = (undefined4 ***)(*(int *)(iVar8 + 4) + *(int *)(iVar9 + 4));
      puStack_3f0 = puStack_3c0;
      (**(code **)((int)*param_1 + 8))(&puStack_3f0);
    }
    fVar3 = param_1[0x146];
    if (*(int *)(*(int *)((int)fVar3 + 0xa8) + 0x38) == 4) {
      puStack_3e8 = (undefined4 *)0xffffffff;
      (**(code **)(*(int *)param_1[0x14b] + 0x15c))(0,&puStack_3e8);
    }
    else {
      iVar8 = *(int *)param_1[0x14b];
      uVar13 = FUN_005140d0(&uStack_3f4,
                            CONCAT22((short)((uint)fVar3 >> 0x10),
                                     *(undefined2 *)
                                      (*(int *)(*(int *)((int)fVar3 + 0xa8) + 0x3c) + 0x4b8)));
      (**(code **)(iVar8 + 0x15c))(0,uVar13);
    }
    (**(code **)(*(int *)param_1[0x14b] + 0x34c))();
    if ((DAT_00d1791c == '\0') && (*(char *)(param_1 + 0x14a) == '\0')) {
      (**(code **)(*(int *)param_1[0x159] + 0x444))(param_1[0x146]);
      return;
    }
    (**(code **)(*(int *)param_1[0x159] + 0xcc))(0);
  }
  return;
}
```

---

## W22-K live seal (2026-07-29)

| Field | Value |
|---|---|
| **Tools** | Ghidra `decompile_function` + `read_memory` (no `disassemble_bytes`) |
| **Program** | `autoassault.exe` base `0x400000` |
| **Body** | `0x0084b890`–`0x0084ca69` (**4569** / `0x11D9` bytes) |
| **Live decompile** | **≡** scaffold raw body (`void __fastcall`, `float *param_1` = `this`) |
| **Xrefs** | **DATA** only `0x00a697e4` (vtbl slot → virtual method) |
| **Callees (Ghidra)** | `FUN_0040d0a0` (Math_AcosClamped), `FUN_00514050`, `FUN_005140d0`, `FUN_00519d20`, `FUN_0052e640`, `FUN_0075bf40`, `FUN_007a69d0`, `FUN_007a6a30`, `FUN_007a6de0`, `FUN_008452f0`, `FUN_00845360`, `FUN_00845880`, `FUN_00845af0`, `FUN_00845b40`, `FUN_00845be0`, `FUN_00847240`, `FUN_0084b210`, `FUN_0084b770`, `FUN_00930ea0`, `UI_BuildItemTooltipStats` `0x00847ad0`, `sprintf` |

### Entry hex (48 bytes)

`
558bec83e4f881ec40010000535556578be983bd18050000000f84b3110000e81cb1f5ff8b9d180500008bb534050000
`

Decode:
- `55 8B EC 83 E4 F8 81 EC 40 01 00 00` — frame; align esp; `sub esp,0x140`
- `53 55 56 57 8B E9` — save; **`this` ECX → EBP**
- `83 BD 18 05 00 00 00` — `cmp dword [ebp+0x518],0` (`this+0x518` = decomp `param_1[0x146]`)
- `0F 84 B3 11 00 00` — jz early return (no bound object)
- `E8 …` — `FUN_007a69d0` (time float)

### Epilogue hex @ `0x0084ca50`

`
… 8B 55 00 6A 00 8B CD FF 92 CC 00 00 00 5F 5E 5D 5B 8B E5 5D C3
`

- vfunc call `[vtbl+0xCC]` path then `pop edi/esi/ebp/ebx; mov esp,ebp; pop ebp; ret`

### Constants (`read_memory`)

| DAT | Float | Role in body |
|---|---|---|
| `DAT_00aaa678` | `≈0.000833` (1/1200) | Y scale with screen `DAT_00d1e81c` |
| `DAT_00aaa67c` | `≈0.000625` (1/1600) | X scale with screen `DAT_00d1e818` |
| `DAT_00aaa7ac` | `100.0` | clamp for `%.01f%%` display |
| `DAT_00aaa99c` | `≈114.5916` (`180/π`) | acos → degrees for gauge |

### Strings (decomp)

- `i_g_2d_overlay_broken.dds`
- `%.01f%% %s`
- `%s: %s`

### Stage map (high level)

1. Gate `this+0x518` object non-null
2. Time + `FUN_0084b210`; set child `+0x14d` field `+0x488 = 1`
3. Subtype short `*(…+0x3f4)==8` → skill mask path vs zero-arg overlay vfunc `+0x1a0` / `FUN_0075bf40`
4. Broken overlay texture vs normal; flags `object+0x17c` bit19 / `DAT_00d1791c`
5. Layout metrics from screen globals × widget `+0x254` / `+0x258` (600) × scale constants
6. Optional percent line (`DAT_00d1791c`) via `FUN_0052e640` + clamp 100 + `sprintf`
7. Title/name lines `%s: %s` / locale helpers
8. Child stack layout (vfuncs `+0x110/118/120/140/13c/268/cc`) including `FUN_00845880` count gate
9. `UI_BuildItemTooltipStats`; angle gauges via `Math_AcosClamped` × deg scale
10. Optional COID table walk on `DAT_00d1b6d8+0x5a0` (8×) sets `DAT_00d1791c`
11. Footer type-4 vs `FUN_005140d0` label; final show/hide vfuncs; return

Live decompile **≡** raw body for CF. Dual A/B: `reviews/A_aa_0084b890_*` / `B_aa_*`.
