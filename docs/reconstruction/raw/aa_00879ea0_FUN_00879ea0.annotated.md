# Annotated low-level: FUN_00879ea0 / UI_ArenaAdvHost_CreateChildWidgets_Inferred

| Field | Value |
|---|---|
| Stable ID | `aa_00879ea0` |
| VA | `0x00879ea0`-`0x0087b38a` (**5355 B** / `0x14EB`) |
| Canonical | `UI_ArenaAdvHost_CreateChildWidgets_Inferred` |
| System | inventory-transfer (UI chrome; no inventory wire) |
| Date | 2026-07-23 scaffold; **2026-08-05 R13-011 dual seal** |

## Machine-level notes (R13-011)

- Source: raw capture for `aa_00879ea0` + live re-verify 2026-08-05.
- Prefer assembly when decompiler conflicts (ABI: thiscall ECX; Ghidra may show `__fastcall`).
- **Dispatch:** sole xref DATA vtbl `0x00a59500` -> this VA (virtual CreateChildWidgets*).
- **Entry:** clear byte `host+0x524`; `FUN_00792600`; SEH `LAB_009b92cd`.
- **Pattern:** `new` + ctor + host `vtbl+0xa8` attach + child `vtbl+0x28` XML `i_d_arena_adv_*` + store slot.
- **Tabs:** `+0x630` console / `+0x634` tournament -> `FUN_00822cb0` + `FUN_00756c90`; `+0x638` advanced -> `FUN_00822cb0` + child `vtbl+0x94`.
- **Tail:** `FUN_00879240`; host `vtbl+0x448`; host `vtbl+0x34c`; `RET`.
- **Not** inventory transfer wire; product class demangle open -> `_Inferred`.
- Dual A/B: accept-with-gaps. Terminal false.
