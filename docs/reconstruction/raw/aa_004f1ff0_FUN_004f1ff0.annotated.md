...
- **Name:** `Object_CollectAllGraphicsPreloads` (plate `"CollectAllGraphicsPreloads"`; Ghidra `FUN_004f1ff0`).
- **ABI:** thiscall ECX=object (`MOV EBP,ECX`); stack `param_2`; `RET 0x4`; void.
- **Body:** `0x004f1ff0`–`0x004f2531` exclusive (1345 B).
- **Semantics:** Profile-bracketed graphics preload collect:
  - Optional `_tint.dds` if clonebase flag bit `0x10` at `+0x3f2`.
  - Optional `_tur.geo` if type `0x0e` or (type `0x12` and field `+0x4e8==1`).
  - If type `0x0e`: lookup clone via `GameData_LookupCloneBaseByCbid` on cbid at clonebase `+0x6f4`; kind@`+0x4ce` selects fork/turret side geo+anm suffixes; then `FUN_004eb010`.
  - Always-ish tail: `FUN_004eb3b0`; if `this+0x88`: `FUN_004a04f0(param_2,1,0,0)` (mission-media map walk).
- **Existence:** `FUN_007b6a20` + `FUN_007b6730` before enqueue (`00989e00` + `0043d670`).
- **Reject:** scaffold `Named_CalleeOf_*` chain.
