...
| **Body** | `0x005f44c0`–`0x005f4ed9` (**2586 B** inclusive end; pad `CC` after) |
| **Prologue** | `55 8B EC 83 E4 F0 6A FF 68 A4 7A 9A 00` — MSVC frame + SEH (`LAB_009a7aa4`) + `SUB ESP,0x168` |
| **Epilogue** | `5B 8B E5 5D C3` (`POP EBX; MOV ESP,EBP; POP EBP; RET`) — **cdecl**, caller cleans |
| **Classification** | worker (large; many callees) |
| **Return** | EAX **0** fail / **1** success (`undefined4` → `uint32_t`) |
| **Callers (1)** | `FUN_005911b0` @ call `0x0059195e` (object just-created / ragdoll flag path) |
| **Xrefs** | 1 unconditional CALL |
| **Integrity** | Live decompile ≡ 2026-07-23 raw body (identical CF; same unreachable-block warnings) |

### Plate strings (product name — sealed)
...
Canonical name: **`GameUtils_createRagDoll`** (Ghidra `FUN_005f44c0`). Prior scaffold aliases `Named_GameUtils_createRagDoll` / mis-tag `Named_CalleeOf_Named_SkyBoxParticles_005f44c0` superseded.

### Sealed ABI (decompiler formals)

```
// cdecl; 3 stack formals; EAX = 0|1
uint32_t GameUtils_createRagDoll(int param_1, int param_2, int param_3);

// param_1 / param_2: buffer-side inputs to FUN_005f4380 (ragdoll parse / bone count)
// param_3: owner/context object:
//   +0x14 → +0x2c → +0xE4A4  world-related id/count (local_164)
//   +0x914 → gfx Body* (required non-null)
//   +0x50 / +0x54 / +0x58 / +0x5c ← outputs on success (built subsystems)
//   +0x60  ← 0x40-byte matrix copy for root bone path
```

Call site in `FUN_005911b0` (bytes @ `0x00591940+`): push parse result + body-side pointer then `CALL 0x005f44c0`; outer object flag `@+0x90d` bit2 set from success bool.

### Control-flow stages (sealed)

1. **Validate:** Body `@param_3+0x914`, world id `local_164`, `param_1`, `param_2` all non-zero → else log invalid-params plate string, return **0**.
2. **Init helpers:** `FUN_006483f0`, `FUN_00649a50`, `FUN_00649d80`, `FUN_006499c0` (SEH state 0..2).
3. **Parse buffer:** `uVar4 = FUN_005f4380(param_1, param_2)` — expected bone/slot count.
4. **Skeleton count:** from Body `+0xA4` range → `local_174`; require `uVar4 < 0x21` (33) and `local_174 < 0x41` (65) else log + fail.
5. **Grow vectors:** `FUN_005d4370` + three `FUN_005f5190` capacity ensures (counts `uVar4` / `local_174`) with `FUN_005b3300` grow helper.
6. **Alloc pose array:** `operator_new[](uVar4*4)` + factory `DAT_00b05060` + `FUN_00404d80(..., 0x20, uVar4)`.
7. **Per-bone loop** over Body bone table (`+0xA4`, count `uStack_17c`):
   - Name from bone `+0xF4→+8` (or `PTR_DAT_00afa2bc`); spaces → `_` via `std::string::find/replace`.
   - `FUN_006482d0(name)` — if non-zero → mark short map `0xFFFF` (skip / already bound).
   - Else register bone index maps; `FUN_00560e90(0)` residual path; pose from `FUN_005f5120` hit → copy float block / `FUN_00567d20`, else identity from `DAT_00bc5630..` + `g_flOne`.
   - Walk parent chain via bone `+0x148` filling parent short map; root bone (`map==0`) copies 4×4 matrix into `param_3+0x60`.
8. **Success gate:** if matched bones `uStack_184 >= uVar15` and loop ran → second pass: `FUN_004e9530` / `FUN_004e96e0` / `FUN_005eadb0` / `FUN_005f50a0` per matched bone; force linked flag `*(child+8)=2` when present.
9. **Build outputs:** factory allocs + `FUN_00648c30`, `FUN_005f5490`×2, `FUN_00648500` → store `param_3+0x50..+0x5c`; teardown vectors; return **1**.
10. **Fail paths:** bones-not-found plate string (`uVar15 - matched`); or limits/params; teardown; return **0**.

### Machine hex (prologue)

```
558bec83e4f06aff68a47a9a0064a100000000506489250000000081ec68010000538b5d10...
```

Epilogue @ `0x005f4ed5`: `5B 8B E5 5D C3`.

### Decompiler warnings (non-blocking)

Unreachable-block removals at `0x005f4b94`, `0x005f4bbf`, `0x005f4ba3`, `0x005f4b62`, `0x005f4d45` — SEH/EH cleanup edges; main success/fail CF sealed.

### Nested callees (OWN residual — not claimed sealed here)

`FUN_005f4380`, `FUN_005f5190`, `FUN_005f5220`, `FUN_005f5120`, `FUN_005f50a0`, `FUN_005f5490`, `FUN_00648c30`, `FUN_00648500`, `FUN_006482d0`, `FUN_005d4370`, `FUN_005d43e0`, `FUN_005b3300`, `FUN_00567d20`, `FUN_004e9530`, `FUN_004e96e0`, `FUN_005eadb0`, `FUN_00560e90`, `FUN_007a4480`, std::string ops, `operator_new[]` / `delete[]`, factory `DAT_00b05060`.
