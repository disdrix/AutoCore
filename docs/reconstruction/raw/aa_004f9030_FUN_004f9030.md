...
| **Image base** | `0x400000` |
| **VA** | `0x004f9030` |
| **Canonical name** | `FUN_004f9030` (structural: `Vehicle_UpdateWeaponHardpoints3_Inferred`) |
...
| **Body range** | `004f9030`–`004f9563` |
| **Classification** | worker |
| **Xrefs** | 2 (`00500612`, `008d7f5e`) |
| **Callers** | `FUN_00500560`, `FUN_008d7e40` |
| **Callees** | `FUN_0056a260`, `FUN_004e9370`, `FUN_00567ce0`, `FUN_004e8830`, `FUN_0056d160`, `FUN_004e88a0`, `FUN_0076eb00`, `FUN_0076f5f0`×2, `FUN_0043f210`, `FUN_0099b8a0`, `FUN_0040cf90` |

### Entry bytes (`read_memory` @ `0x004f9030` length 64 excerpt)

```
55 8b ec 83 e4 f0 81 ec e4 00 00 00 53 56 8b f1
; push ebp; mov ebp,esp; and esp,-16; sub esp,0xe4; push ebx; push esi; mov esi,ecx
8b 46 04 8b 48 04 8b 84 31 b0 00 00 00
; this+4 → +4 → this+0xb0 lock/control object
...
8b 96 60 02 00 00 8b 3c 13   ; weapon table this+0x260; slot = [table+index]
6a 00 8b cf e8 .. .. .. ..   ; push 0; mov ecx,slot; call Weapon_SetFireFlagPair(0)
c6 87 c9 00 00 00 00         ; slot+0xc9 = 0
```

### Epilogue (`read_memory` @ `0x004f9560`)

```
8b e5 5d c3                  ; mov esp,ebp; pop ebp; ret  (near; 0 stack args cleaned)
```
...
| Register / stack | Role |
|---|---|
| **ECX** | vehicle `this` (saved to ESI) |
| Stack args | none (near `RET`) |
| Return | void |
...
|---|---|
| `DAT_00af18b0` | `00 00 00 00  00 00 80 3f  00 00 00 00  00 00 00 00` → axis `(0, 1.0f, 0, 0)` (+Y) for `Quat_FromAxisAngle` |
| `DAT_00b041b0` | BSS zeros (fallback fire-helper host when `this+8==0`) |
